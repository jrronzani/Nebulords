   ...
   ;  Version 49 - Fix OOB detection for caught balls
   ;  Based on v48 (out-of-bounds reset)
   ;
   ;  P2 starting position set to 108 for visual symmetry
   ;  Ball respawns at top center if out of bounds for 1 second
   ;  Ball flashes for 2 seconds before becoming live (safe spawn)
   ;  OOB detection now works for caught balls too
   ;
...
__Skip_Ball_Move
   ...
   ; Ball vs player cores - 1-hit kill (skip during warning flash)
   if ball_state = 0 && ball_warning = 0 && p1_alive && collision(ball,player0) then gosub __P1_Destroyed
   if ball_state = 0 && ball_warning = 0 && p2_alive && collision(ball,player1) then gosub __P2_Destroyed
...
__Check_OOB
   ...
   ; Using INNER boundaries so OOB triggers before "keep in bounds" snap
   if ballx < 12 then goto __Ball_OOB
   if ballx > 148 then goto __Ball_OOB
   if bally < 12 then goto __Ball_OOB
   if bally > 83 then goto __Ball_OOB
...
__Warning_Flash
   ...
   ; Flash ball by alternating ballheight (4 frames on, 4 frames off)
   temp1 = ball_warning & 4  ; Check bit 2 (cycles every 8 frames)
   if temp1 then ballheight = 1 else ballheight = 0
   return
