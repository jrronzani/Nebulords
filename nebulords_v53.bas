   ...
   ;  Version 53 - Fix shield bug with simpler bit checking
   ;  Based on v52 (shield system)
   ;
   ;  P2 starting position set to 108 for visual symmetry
   ;  Ball respawns at top center if out of bounds for 1 second
   ;  Shield system simplified with direct bit checks (no shift loop)
   ;  Added cooldown after shield break to prevent double-hits
   ;
...
__P1_Check_Shield
   ...
   ; Check if that shield bit is set using direct bit checks
   if temp5 = 2 then goto __P1_Check_North
   if temp5 = 3 then goto __P1_Check_East
   if temp5 = 4 then goto __P1_Check_South
   if temp5 = 5 then goto __P1_Check_West
   goto __P1_Destroyed  ; Safety fallback

__P1_Check_North
   if p1_state{2} then p1_state{2} = 0 : goto __P1_Shield_Bounce
   goto __P1_Destroyed

__P1_Check_East
   if p1_state{3} then p1_state{3} = 0 : goto __P1_Shield_Bounce
   goto __P1_Destroyed

__P1_Check_South
   if p1_state{4} then p1_state{4} = 0 : goto __P1_Shield_Bounce
   goto __P1_Destroyed

__P1_Check_West
   if p1_state{5} then p1_state{5} = 0 : goto __P1_Shield_Bounce
   goto __P1_Destroyed

__P1_Shield_Bounce
   ; Shield broken - bounce ball away and add brief invulnerability
   ball_xvel = 0 - ball_xvel
   ball_yvel = 0 - ball_yvel
   ballx = ballx + ball_xvel
   bally = bally + ball_yvel
   ; Brief invulnerability to prevent double-hit
   ball_warning = 10  ; 10 frames of invulnerability
   return
...
__P2_Check_Shield
   ; temp5 now contains shield bit position (2-5)
   ; Check if that shield bit is set using direct bit checks
   if temp5 = 2 then goto __P2_Check_North
   if temp5 = 3 then goto __P2_Check_East
   if temp5 = 4 then goto __P2_Check_South
   if temp5 = 5 then goto __P2_Check_West
   goto __P2_Destroyed  ; Safety fallback

__P2_Check_North
   if p2_state{2} then p2_state{2} = 0 : goto __P2_Shield_Bounce
   goto __P2_Destroyed

__P2_Check_East
   if p2_state{3} then p2_state{3} = 0 : goto __P2_Shield_Bounce
   goto __P2_Destroyed

__P2_Check_South
   if p2_state{4} then p2_state{4} = 0 : goto __P2_Shield_Bounce
   goto __P2_Destroyed

__P2_Check_West
   if p2_state{5} then p2_state{5} = 0 : goto __P2_Shield_Bounce
   goto __P2_Destroyed
