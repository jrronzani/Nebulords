   ...
   ;  Version 41 - Ball lookahead collision to prevent paddle penetration
   ;  Based on v36 (stable milestone)
   ;
   ;  Player movement: v36 bounce system (asteroids-style planned for future)
   ;  Ball collision: Proactive lookahead prevents penetration through paddles
   ;
...
__Death_Ball_Movement
   ;***************************************************************
   ;  Handle ball movement with LOOKAHEAD collision detection
   ;  Prevents ball from penetrating through paddles at high speed
   ;***************************************************************
...
__Hide_P2_Paddle
   ...
   ;***************************************************************
   ;  Paddle collision handling with LOOKAHEAD
   ;  Ball is already at its NEXT position when these are called
   ;***************************************************************

__Ball_Hit_P1_Paddle_Lookahead
   ...
   ; Otherwise bounce: reverse velocity and keep ball at current (future) position
   ; Lookahead prevents penetration, normal bounce pushes ball away
   ball_xvel = 0 - ball_xvel
   ball_yvel = 0 - ball_yvel
   ; Ball is already at next position from lookahead, velocity reversed means it moves away next frame
   return

__Ball_Hit_P2_Paddle_Lookahead
