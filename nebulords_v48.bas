   ...
   ;  Version 48 - Out-of-bounds reset system
...
__Skip_Ball_Move
   ...
   ;***************************************************************
   ; Check if ball is out of bounds (only when free)
   if ball_state = 0 && ball_warning = 0 then gosub __Check_OOB
...
__Check_OOB
   ...
   if ballx < 8 then goto __Ball_OOB
   if ballx > 152 then goto __Ball_OOB
   if bally < 8 then goto __Ball_OOB
   if bally > 87 then goto __Ball_OOB
