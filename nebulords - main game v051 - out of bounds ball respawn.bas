   ...
   ;  Version 51 - Tighter side OOB boundaries
   ;  Based on v50 (side OOB fix)
   ;
   ;  P2 starting position set to 108 for visual symmetry
   ;  Ball respawns at top center if out of bounds for 1 second
   ;  Ball flashes 2 on/6 off for obvious blinking
   ;  Side OOB boundaries very aggressive (x<20, x>140)
   ;  Left/Right now trigger much earlier to prevent timer reset
   ;
