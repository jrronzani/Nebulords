   ...
   ;  Version 37 - Add independent X/Y collision for diagonal sliding
   ;  Based on v36 with improved player movement
   ;
...
__Main_Loop
   ...
   ;***************************************************************
   ;  Handle Player 1 controls - Independent X/Y for wall-sliding
   ;***************************************************************
   ; Only process if alive
   if !p1_alive then goto __Skip_P1_Controls

   ; X axis: Read input, apply movement, check collision, undo if hit
   p1_xspeed = 0
   if joy0right then p1_xspeed = PLAYER_SPEED
   if joy0left then p1_xspeed = 0 - PLAYER_SPEED
   player0x = player0x + p1_xspeed
   if p1_xspeed && collision(player0, playfield) then player0x = player0x - p1_xspeed

   ; Y axis: Read input, apply movement, check collision, undo if hit
   p1_yspeed = 0
   if joy0down then p1_yspeed = PLAYER_SPEED
   if joy0up then p1_yspeed = 0 - PLAYER_SPEED
   player0y = player0y + p1_yspeed
   if p1_yspeed && collision(player0, playfield) then player0y = player0y - p1_yspeed
...
__Skip_P1_Controls
   ...
   ;***************************************************************
   ;  Handle Player 2 controls - Independent X/Y for wall-sliding
   ;***************************************************************
   ...
   if p2_xspeed && collision(player1, playfield) then player1x = player1x - p2_xspeed
   ...
   if p2_yspeed && collision(player1, playfield) then player1y = player1y - p2_yspeed
