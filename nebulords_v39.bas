   ...
   ;  Version 39 - Missile-based collision lookahead
   ;  Based on v36 (stable milestone)
   ;
   ;  Uses missiles to check for walls BEFORE moving player
   ;  Independent X/Y axis checking for smooth diagonal wall-sliding
   ;
...
__Main_Loop
   ...
   ;***************************************************************
   ;  Handle Player 1 controls - Missile-based collision lookahead
   ;  X and Y checked independently for diagonal wall-sliding
   ;***************************************************************
   ; Only process if alive
   if !p1_alive then goto __Skip_P1_Controls

   ; X axis movement with missile lookahead
   p1_xspeed = 0
   if joy0right then p1_xspeed = PLAYER_SPEED
   if joy0left then p1_xspeed = 0 - PLAYER_SPEED

   ; Only check collision if trying to move
   if p1_xspeed then temp1 = missile0x : missile0x = missile0x + p1_xspeed : if !collision(missile0, playfield) then player0x = player0x + p1_xspeed
   if p1_xspeed then missile0x = temp1

   ; Y axis movement with missile lookahead
   p1_yspeed = 0
   if joy0down then p1_yspeed = PLAYER_SPEED
   if joy0up then p1_yspeed = 0 - PLAYER_SPEED

   ; Only check collision if trying to move
   if p1_yspeed then temp1 = missile0y : missile0y = missile0y + p1_yspeed : if !collision(missile0, playfield) then player0y = player0y + p1_yspeed
   if p1_yspeed then missile0y = temp1
...
__Skip_P1_Controls
   ...
   ;***************************************************************
   ;  Handle Player 2 controls - Missile-based collision lookahead
   ;***************************************************************
   ...
   if p2_xspeed then temp2 = missile1x : missile1x = missile1x + p2_xspeed : if !collision(missile1, playfield) then player1x = player1x + p2_xspeed
   if p2_xspeed then missile1x = temp2
   ...
   if p2_yspeed then temp2 = missile1y : missile1y = missile1y + p2_yspeed : if !collision(missile1, playfield) then player1y = player1y + p2_yspeed
   if p2_yspeed then missile1y = temp2
