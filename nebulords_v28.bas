   ...
   ;  Version 28 - Fix auto-launch repeated firing bug
   ;  Prevent button from influencing ball after auto-launch
   ;***************************************************************
...
__P1_Button_Held
   ...
   ; Check for max hold (5 seconds = 300 frames) - only if not already in cooldown
   if p1_timer > 300 && !p1_state{1} then gosub __P1_Auto_Launch
...
__P2_Button_Held
   ...
   ; Check for max hold (5 seconds = 300 frames) - only if not already in cooldown
   if p2_timer > 300 && !p2_state{1} then gosub __P2_Auto_Launch
   p2_state{0} = 1
   return
