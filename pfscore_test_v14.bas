   ;***************************************************************
   ;  PFSCORE TEST V14 - Playfield Collision Detection
   ;
   ...
   ;
   ;  NEW: Real playfield collision detection!
   ;  - Uses collision(player0,playfield) for accurate detection
   ;  - Works with ANY playfield shape, not just boundaries
   ;  - Can add obstacles, blocks, mazes, etc.
   ;  - Bounces off any playfield pixel
   ;
   ...
   dim x_moved = o                ; Flag: did X move this frame?
   dim y_moved = p                ; Flag: did Y move this frame?
...
__Main_Loop
   ...
   ;***************************************************************
   ;  Check Playfield Collisions and Bounce
   ;***************************************************************
   gosub __Check_Playfield_Collision
...
__Apply_Movement
   ...
   if frame_counter_x = 0 then player0x = player0x + dir_x : frame_counter_x = speed_x : x_moved = 1

   ; Apply Y movement (frame-based velocity)
   if dir_y <> 0 then frame_counter_y = frame_counter_y - 1
   if frame_counter_y = 0 then player0y = player0y + dir_y : frame_counter_y = speed_y : y_moved = 1

   return


__Check_Playfield_Collision
   ; Check if player collided with playfield
   if !collision(player0,playfield) then return

   ; Collision detected! Determine which axis and bounce

   ; If X moved this frame, reverse X direction
   if x_moved then player0x = prev_x : dir_x = 0 - dir_x : frame_counter_x = speed_x

   ; If Y moved this frame, reverse Y direction
   if y_moved then player0y = prev_y : dir_y = 0 - dir_y : frame_counter_y = speed_y
