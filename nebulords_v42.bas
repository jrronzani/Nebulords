   ...
   ;  Version 42 - Bounce speed boost prevents paddle penetration
   ;  Based on v36 (stable milestone)
   ;
   ;  Ball gets 3-frame speed boost on bounce (prevents paddle overtaking)
   ;  Similar to launch mechanic but shorter duration
   ;
...
__Ball_Hit_P1_Paddle
   ...
   ; Otherwise BOUNCE with speed boost (prevents paddle overtaking)
   ; Reverse and double velocity for 3-frame boost
   ball_xvel = 0 - ball_xvel
   ball_yvel = 0 - ball_yvel
   ball_xvel = ball_xvel + ball_xvel  ; Double X velocity
   ball_yvel = ball_yvel + ball_yvel  ; Double Y velocity
   ball_speed_timer = BOUNCE_BOOST_DURATION  ; 3 frames
