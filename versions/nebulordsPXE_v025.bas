  ;***************************************************************
  ;  NEBULORDS PXE - Version 025
  ;  Warlords-style Space Combat with Paddle Controls
  ;
  ;  Changes from v024:
  ;  - FIRST-PRESS DETECTION: Tapping button triggers instant response!
  ;  - Held button continues with accel_delay (snappy feel maintained)
  ;  - Start speed: 16 (was 20) - 1 pixel every 16 frames when slowest
  ;  - Step size: 2 (unchanged)
  ;  - Acceleration path: 16->14->12->10->8->6->4->2 (exactly 8 steps)
  ;  - 1 tap = 1 speed step change
  ;  - At max speed, 8 taps opposite direction = perfect stop
  ;
  ;  Physics System:
  ...
  ;  - First button press = instant thrust, held = with accel_delay
  ...
  ;  - Paddle 0 button (joy0right): Tap for instant thrust, hold to continue
  ;  - Paddle 1 button (joy0left): Tap for instant thrust, hold to continue
  ;***************************************************************
  ...
  dim p1_button_prev = y         ; P1 button state last frame
  dim p2_button_prev = z         ; P2 button state last frame
...
__Main_Loop
  ...
  ;***************************************************************
  ;  Thrust Physics - First press immediate, held with delay
  ;***************************************************************
  ; Player 1: First press (instant) OR held (with accel_delay)
  if joy0right && !p1_button_prev then temp_dir = p1_direction : gosub __P1_Thrust
  if joy0right && p1_button_prev && accel_counter = 0 then temp_dir = p1_direction : gosub __P1_Thrust

  ; Player 2: First press (instant) OR held (with accel_delay)
  if joy0left && !p2_button_prev then temp_dir = p2_direction : gosub __P2_Thrust
  if joy0left && p2_button_prev && accel_counter = 0 then temp_dir = p2_direction : gosub __P2_Thrust
