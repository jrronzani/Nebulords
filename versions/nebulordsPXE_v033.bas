  ;***************************************************************
  ;  NEBULORDS PXE - Version 033
  ;  Warlords-style Space Combat with Paddle Controls
  ;
  ;  Changes from v030:
  ;  - FIXED: Compilation errors in collision detection (removed spaces, split statements)
  ;  - BRICK HITBOX DETECTION: Y-position based detection for 4 bricks per player
  ;  - BRICK DESTRUCTION: Bit flags track brick state (bit 0=top, 1=left, 2=right, 3=bottom)
  ;  - SPRITE ANIMATION: Dynamic sprite updates show destroyed bricks (states 0 and 15 implemented)
  ;  - CORE HIT DETECTION: Ball hitting destroyed brick area = core hit
  ;  - PLAYER DEATH: On core hit, player resets to starting position with all bricks restored
  ;  - Ball bounces off bricks and core hits
  ;  - Next: Score tracking and round end detection
  ;  NOTE: Using ORIGINAL sprite design from v028-v030 (v031 sprite changes reverted)
  ;
  ;  Physics System:
...
__P1_Core_Hit
  ; Player 1 dies - reset position and restore all bricks
  player0x = 25 : player0y = 35
  p1_bricks = %00001111  ; Restore all bricks
...
__P2_Core_Hit
  ; Player 2 dies - reset position and restore all bricks
  player1x = 117 : player1y = 35
  p2_bricks = %00001111  ; Restore all bricks
