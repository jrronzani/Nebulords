  ;***************************************************************
  ;  NEBULORDS PXE - Version 024
  ;  Warlords-style Space Combat with Paddle Controls
  ;
  ;  Changes from v021:
  ;  - Slower starting speed for finer tap control
  ;  - Start speed: 20 (was 10) - 1 pixel every 20 frames when slow
  ;  - Step size: 2 (was 1) - bigger jumps but still ~10 steps
  ;  - Stopping threshold: >= 20 (was >= 10)
  ;  - Testing if slower "slowest speed" helps with precision taps
  ;
  ;  Physics System:
  ;  - speed_x/y: Frames between pixel moves (lower = faster)
  ;  - max_speed = 2: Top speed (moves every 2 frames)
  ;  - Start at 20, step by 2: 20->18->16->14->12->10->8->6->4->2
...
__Game_Init
  ...
  p1_speed_x = 20 : p1_speed_y = 20
  p1_frame_x = 20 : p1_frame_y = 20
  p1_dir_x = 0 : p1_dir_y = 0

  ; Initialize P2 physics - start stationary
  p2_speed_x = 20 : p2_speed_y = 20
  p2_frame_x = 20 : p2_frame_y = 20
...
__P1_Accel_Right
  ...
  if p1_dir_x = 255 then p1_speed_x = p1_speed_x + 2 : if p1_speed_x >= 20 then p1_dir_x = 0 : p1_speed_x = 20
...
__P1_Accel_Left
  ...
  if p1_dir_x = 1 then p1_speed_x = p1_speed_x + 2 : if p1_speed_x >= 20 then p1_dir_x = 0 : p1_speed_x = 20
...
__P1_Accel_Up
  ...
  if p1_dir_y = 1 then p1_speed_y = p1_speed_y + 2 : if p1_speed_y >= 20 then p1_dir_y = 0 : p1_speed_y = 20
...
__P1_Accel_Down
  ...
  if p1_dir_y = 255 then p1_speed_y = p1_speed_y + 2 : if p1_speed_y >= 20 then p1_dir_y = 0 : p1_speed_y = 20
...
__P2_Accel_Right
  if p2_dir_x = 255 then p2_speed_x = p2_speed_x + 2 : if p2_speed_x >= 20 then p2_dir_x = 0 : p2_speed_x = 20
  if p2_dir_x = 0 then p2_dir_x = 1
  if p2_dir_x = 1 then if p2_speed_x > max_speed then p2_speed_x = p2_speed_x - 2
  return

__P2_Accel_Left
  if p2_dir_x = 1 then p2_speed_x = p2_speed_x + 2 : if p2_speed_x >= 20 then p2_dir_x = 0 : p2_speed_x = 20
  if p2_dir_x = 0 then p2_dir_x = 255
  if p2_dir_x = 255 then if p2_speed_x > max_speed then p2_speed_x = p2_speed_x - 2
  return

__P2_Accel_Up
  if p2_dir_y = 1 then p2_speed_y = p2_speed_y + 2 : if p2_speed_y >= 20 then p2_dir_y = 0 : p2_speed_y = 20
  if p2_dir_y = 0 then p2_dir_y = 255
  if p2_dir_y = 255 then if p2_speed_y > max_speed then p2_speed_y = p2_speed_y - 2
  return

__P2_Accel_Down
  if p2_dir_y = 255 then p2_speed_y = p2_speed_y + 2 : if p2_speed_y >= 20 then p2_dir_y = 0 : p2_speed_y = 20
