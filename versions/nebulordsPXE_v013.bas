  ;***************************************************************
  ;  NEBULORDS PXE - Version 013
  ;  Warlords-style Space Combat with Paddle Controls
  ;
  ;  Changes from v012:
  ;  - FIXED collision boundaries to account for sprite dimensions
  ;  - Sprites now flush against borders (no sinking in or gaps)
  ;  - Calculated based on: 26-scanline tall × 16-pixel wide sprites
  ;  - Left boundary: 4 (past left wall)
  ;  - Right boundary: 140 (sprite extends to 156, before right wall)
  ;  - Top boundary: 8 (past top border)
  ;  - Bottom boundary: 135 (sprite extends to 161, before bottom border)
  ;
  ;  From v012:
...
__Main_Loop
  ...
  ;***************************************************************
  ;  Boundary checking for Players (FLUSH - no cushion)
  ;  Sprite: 26 scanlines tall × 16 pixels wide
  ;  playerXx = left edge, playerXy = top edge
  ;***************************************************************
  if p1_xpos < 4 then p1_xpos = 4
  if p1_xpos > 140 then p1_xpos = 140
  if p1_ypos < 8 then p1_ypos = 8
  if p1_ypos > 135 then p1_ypos = 135

  if p2_xpos < 4 then p2_xpos = 4
  if p2_xpos > 140 then p2_xpos = 140
  if p2_ypos < 8 then p2_ypos = 8
  if p2_ypos > 135 then p2_ypos = 135

  ;***************************************************************
  ;  Ball physics - DOUBLED Y velocities to match X velocities
  ;  Ball: 1 pixel wide × 4 scanlines tall
  ;***************************************************************
