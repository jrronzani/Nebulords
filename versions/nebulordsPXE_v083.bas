  ;***************************************************************
  ;  NEBULORDS PXE - Version 083
  ;  Warlords-style Space Combat with Paddle Controls
  ;
  ;  Changes from v082:
  ;  - REMOVE: pfscore bar display (not supported in PXE kernel)
  ;  - REMOVE: const pfscore = 1 (causes assembly errors in PXE)
  ;  - FIX: Disable visual score display to avoid label mismatch errors
  ;  - KEEP: Internal win tracking (p1_wins, p2_wins) without display
  ;
  ;  Changes from v079:
...
__Draw_Scores
  ; DISABLED - PXE kernel doesn't support pfscore bars like DPC+ kernel
  ; Win tracking still works internally (p1_wins, p2_wins)
  ; Visual display would require different approach
  return
