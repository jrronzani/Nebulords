...
__Game_Init
  ...
  ; Paddle colors - gradient matching ship sprites
  player2color:
  $90
  $92
  $94
  ...
  ;***************************************************************
__Main_Loop
  ; DEBUG v052: Reset background to black each frame (will flash red if __P2_Brick_Hit is called)
  COLUBK = $00
...
__P2_Button_Done
  ...
  ; Check P1 paddle (player2) - Ball: 2x4, Paddle: 6x9, Hitbox: 8x11 (expanded +1px each side)
  if ballx < player2x + 7 && ballx + 2 > player2x - 1 then if bally < player2y + 10 && bally + 4 > player2y - 1 then gosub __Check_P1_Paddle
...
__Skip_Paddle_Collision
  ...
  ;  Skip during invincibility, if player is off-screen, or cooldown after paddle bounce
  ;  NOTE: Check one player per frame to prevent cross-player brick breaking
  ;  DEBUG v052: Disabled ship_collision_cooldown check to test cross-contamination
  ;***************************************************************
  if invincibility_timer > 0 then goto __Skip_Ball_Physics
  ; DEBUG: Commented out to test if cooldown is the issue
  ; if ship_collision_cooldown > 0 then goto __Skip_Ball_Physics
...
__Ball_Bounce_P2
  ...
  ;***************************************************************
__P1_Brick_Hit
  ; DEBUG v052: Flash background GREEN when this function is called
  COLUBK = $C4
  ; Check immunity - if P1 is immune, skip brick destruction entirely
  if p1_brick_immunity > 0 then return
  ; Sprite is 26 scanlines tall, drawn top to bottom
  ; Ball is 4 pixels tall
  ; Check BOTTOM of ball to determine which brick was hit
  ; Hitboxes expanded +1px to prevent pixel-perfect penetration
  ; Bottom brick: Ball bottom in Y offset 20-25 (expanded from 21, visual bottom of sprite)
  if bally + 4 > player0y + 20 then goto __P1_Bottom_Brick
  ; Middle section (left/core/right): Ball bottom in Y offset 6-21 (expanded ±1px)
  if bally + 4 > player0y + 6 then goto __P1_Middle_Brick
  ; Top brick: Ball bottom in Y offset 0-7 (expanded from 6, visual top of sprite)
  goto __P1_Top_Brick

__P1_Top_Brick
  ; Check if brick already destroyed
  if !p1_bricks{0} then goto __P1_Top_Check_Core
  ; Brick intact - destroy it and bounce
  p1_bricks{0} = 0
  goto __P1_Brick_Bounce

__P1_Top_Check_Core
  ; Top brick destroyed - check if ball is hitting exposed core
  ; Core Y range: player0y + 7 to player0y + 20
  ; Core X range: player0x + 5 to player0x + 11
  if bally + 4 < player0y + 7 then return  ; Ball above core
  if bally > player0y + 20 then return      ; Ball below core
  if ballx + 2 < player0x + 5 then return  ; Ball left of core
  if ballx > player0x + 11 then return      ; Ball right of core
  goto __P1_Core_Hit  ; Ball is hitting exposed core!

__P1_Middle_Brick
  ; Determine left vs core vs right based on X position
  ; Sprite is ~16 pixels wide: left brick (0-4), core (5-10), right brick (11-15)
  ; Ball is 2 pixels wide - use overlap detection
  ; Core X range: player0x + 5 to player0x + 11 (6 pixels wide)
  ; Check CORE FIRST (highest priority) - dies if any part of ball overlaps core
  ; Ball overlaps core if: ball right edge (ballx+2) > core left (5) AND ball left edge (ballx) < core right (11)
  if ballx + 2 > player0x + 5 then if ballx < player0x + 11 then goto __P1_Core_Hit
  ; Left brick: ball is entirely left of core
  if ballx + 2 <= player0x + 5 then goto __P1_Left_Brick
  ; Right brick: ball is entirely right of core
  goto __P1_Right_Brick

__P1_Left_Brick
  if !p1_bricks{2} then goto __P1_Left_Check_Core
  p1_bricks{2} = 0
  goto __P1_Brick_Bounce

__P1_Left_Check_Core
  ; Left brick destroyed - check if ball is hitting exposed core
  ; Core X range: player0x + 5 to player0x + 11
  if ballx + 2 < player0x + 5 then return  ; Ball left of core
  if ballx > player0x + 11 then return      ; Ball right of core
  goto __P1_Core_Hit  ; Ball is hitting exposed core!
...
__P1_Right_Brick
  if !p1_bricks{1} then goto __P1_Right_Check_Core
  p1_bricks{1} = 0
  goto __P1_Brick_Bounce

__P1_Right_Check_Core
  ; Right brick destroyed - check if ball is hitting exposed core
  ; Core X range: player0x + 5 to player0x + 11
  if ballx + 2 < player0x + 5 then return  ; Ball left of core
  if ballx > player0x + 11 then return      ; Ball right of core
  goto __P1_Core_Hit  ; Ball is hitting exposed core!
...
__P1_Bottom_Brick
  if !p1_bricks{3} then goto __P1_Bottom_Check_Core
  p1_bricks{3} = 0
  goto __P1_Brick_Bounce

__P1_Bottom_Check_Core
  ; Bottom brick destroyed - check if ball is hitting exposed core
  ; Core Y range: player0y + 7 to player0y + 20
  ; Core X range: player0x + 5 to player0x + 11
  if bally + 4 < player0y + 7 then return  ; Ball above core
  if bally > player0y + 20 then return      ; Ball below core
...
__P2_Brick_Hit
  ; DEBUG v052: Flash background RED when this function is called
  COLUBK = $42
  ...
  ; Ball is 4 pixels tall
  ; Check BOTTOM of ball to determine which brick was hit
  ; NOTE: P2 hitboxes shifted +2 pixels right to align with sprite
  ; Hitboxes expanded +1px to prevent pixel-perfect penetration
  ; Bottom brick: Ball bottom in Y offset 20-25 (expanded from 21, visual bottom of sprite)
  if bally + 4 > player1y + 20 then goto __P2_Bottom_Brick
  ; Middle section (left/core/right): Ball bottom in Y offset 6-21 (expanded ±1px)
  if bally + 4 > player1y + 6 then goto __P2_Middle_Brick
  ; Top brick: Ball bottom in Y offset 0-7 (expanded from 6, visual top of sprite)
  goto __P2_Top_Brick

__P2_Top_Brick
  ; Check if brick already destroyed
  if !p2_bricks{0} then goto __P2_Top_Check_Core
  ; Brick intact - destroy it and bounce
  p2_bricks{0} = 0
...
__P2_Top_Check_Core
  ; Top brick destroyed - check if ball is hitting exposed core
  ; Core Y range: player1y + 7 to player1y + 20
  ; Core X range: player1x + 7 to player1x + 13 (P2 shifted +2)
  if bally + 4 < player1y + 7 then return  ; Ball above core
  if bally > player1y + 20 then return      ; Ball below core
  if ballx + 2 < player1x + 7 then return  ; Ball left of core
  if ballx > player1x + 13 then return      ; Ball right of core
  goto __P2_Core_Hit  ; Ball is hitting exposed core!

__P2_Middle_Brick
  ; Determine left vs core vs right based on X position
  ; NOTE: P2 hitboxes shifted +2 pixels right to align with sprite
  ; Sprite is ~16 pixels wide: left brick (0-6), core (7-12), right brick (13-15)
  ; Ball is 2 pixels wide - use overlap detection
  ; Core X range: player1x + 7 to player1x + 13 (6 pixels wide, shifted +2)
  ; Check CORE FIRST (highest priority) - dies if any part of ball overlaps core
  ; Ball overlaps core if: ball right edge (ballx+2) > core left (7) AND ball left edge (ballx) < core right (13)
  if ballx + 2 > player1x + 7 then if ballx < player1x + 13 then goto __P2_Core_Hit
  ; Left brick: ball is entirely left of core
  if ballx + 2 <= player1x + 7 then goto __P2_Left_Brick
  ; Right brick: ball is entirely right of core
  goto __P2_Right_Brick

__P2_Left_Brick
  if !p2_bricks{2} then goto __P2_Left_Check_Core
  p2_bricks{2} = 0
  goto __P2_Brick_Bounce

__P2_Left_Check_Core
  ; Left brick destroyed - check if ball is hitting exposed core
  ; Core X range: player1x + 7 to player1x + 13 (P2 shifted +2)
  if ballx + 2 < player1x + 7 then return  ; Ball left of core
  if ballx > player1x + 13 then return      ; Ball right of core
...
__P2_Right_Brick
  if !p2_bricks{1} then goto __P2_Right_Check_Core
  p2_bricks{1} = 0
  goto __P2_Brick_Bounce

__P2_Right_Check_Core
  ; Right brick destroyed - check if ball is hitting exposed core
  ; Core X range: player1x + 7 to player1x + 13 (P2 shifted +2)
  if ballx + 2 < player1x + 7 then return  ; Ball left of core
  if ballx > player1x + 13 then return      ; Ball right of core
...
__P2_Bottom_Brick
  if !p2_bricks{3} then goto __P2_Bottom_Check_Core
  p2_bricks{3} = 0
  goto __P2_Brick_Bounce

__P2_Bottom_Check_Core
  ; Bottom brick destroyed - check if ball is hitting exposed core
  ; Core Y range: player1y + 7 to player1y + 20
  ; Core X range: player1x + 7 to player1x + 13 (P2 shifted +2)
  if bally + 4 < player1y + 7 then return  ; Ball above core
