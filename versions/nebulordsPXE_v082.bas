  ;***************************************************************
  ;  NEBULORDS PXE - Version 082
  ;  Warlords-style Space Combat with Paddle Controls
  ;
  ;  Changes from v081:
  ;  - FIX: Implement pfscore bars for win counter display (like StarGiraf lives)
  ;  - ADD: Binary pattern score display using pfscore1/pfscore2
  ;  - REPLACE: pfpixel approach with pfscore bars (fixes assembly errors)
  ;  - P1 wins shown on left pfscore bar (blue)
  ;  - P2 wins shown on right pfscore bar (purple)
  ;
  ;  Changes from v079:
  ;  - REPLACE: Custom playfield score display in 4 corners
  ;  - REMOVE: Built-in score system (didn't work with PXE)
  ;  - ADD: 4-player win tracking (p1_wins, p2_wins, p3_wins, p4_wins)
  ;  - ADD: Corner score blocks - P1 top-left, P2 top-right, P3 bottom-left, P4 bottom-right
  ;  - Each player's blocks grow toward center (max 5 wins)
  ...
  const pfscore = 1  ; Enable pfscore bars for win counter display
  ...
  ;***************************************************************
  ;  WIN TRACKING - 4 Players, First to 5 Wins
  ;  Displayed as colored blocks in playfield corners
  ;***************************************************************
  dim p1_wins = var2             ; P1 wins (0-5, blue, top-left)
  dim p2_wins = var3             ; P2 wins (0-5, purple, top-right)
  dim p3_wins = var7             ; P3 wins (0-5, orange, bottom-left)
  dim p4_wins = var8             ; P4 wins (0-5, green, bottom-right)
...
__Game_Init
  ...
  ; Initialize win counters and round state
  p1_wins = 0 : p2_wins = 0 : p3_wins = 0 : p4_wins = 0
  invincibility_timer = 0    ; No invincibility at start

  ; Initialize pfscore bars for win display
  pfscorecolor = $6E          ; Blue for P1 (will alternate with P2 purple)
  pfscore1 = %00000000        ; P1 wins (left bar) - starts empty
  pfscore2 = %00000000        ; P2 wins (right bar) - starts empty
  gosub __Draw_Scores         ; Initialize score bar display
...
__P1_Core_Hit
  ...
  gosub __Award_P2_Win           ; Award win to Player 2
...
__P2_Core_Hit
  ...
  gosub __Award_P1_Win           ; Award win to Player 1
...
__Award_P1_Win
  ...
  gosub __Draw_Scores  ; Update pfscore bars
  return
...
__Draw_Scores
  ; Update pfscore bars to display win counts using binary patterns
  ; Similar to StarGiraf lives counter implementation

  ; Update P1 wins (left pfscore bar)
  if p1_wins = 0 then pfscore1 = %00000000
  if p1_wins = 1 then pfscore1 = %10000000
  if p1_wins = 2 then pfscore1 = %10100000
  if p1_wins = 3 then pfscore1 = %10101000

  ; Update P2 wins (right pfscore bar)
  if p2_wins = 0 then pfscore2 = %00000000
  if p2_wins = 1 then pfscore2 = %10000000
  if p2_wins = 2 then pfscore2 = %10100000
  if p2_wins = 3 then pfscore2 = %10101000

  return

__Game_Over
  ; Game over - someone reached 5 wins
  ; TODO: Display winner and reset
  return
...
__Setup_Playfield
  ...
  ;***************************************************************
  ;  Score Display - Playfield Corner Blocks
  ;  P1 (blue): Top-left, grows right
  ;  P2 (purple): Top-right, grows left
  ;  P3 (orange): Bottom-left, grows right
  ;  P4 (green): Bottom-right, grows left
  ;***************************************************************
  ; Score blocks drawn dynamically in playfield using pfpixel
  ; Updated in main loop based on p1_wins, p2_wins, p3_wins, p4_wins
