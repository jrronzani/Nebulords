...
__P1_Top_Check_Core
  ...
  ; If ball misses core, check if it can hit other intact bricks
  if bally + 4 >= player0y + 7 then if bally <= player0y + 20 then if ballx + 2 >= player0x + 5 then if ballx <= player0x + 11 then goto __P1_Core_Hit
  ; Ball missed core - try to hit bottom brick if intact
  if p1_bricks{3} then goto __P1_Bottom_Brick
  ; Try left/right bricks
  if p1_bricks{2} then goto __P1_Left_Brick
  if p1_bricks{1} then goto __P1_Right_Brick
  ; No brick to hit - pass through destroyed area
  return

__P1_Middle_Brick
  ; Determine left vs core vs right based on X position
  ; Sprite is ~16 pixels wide: left brick (0-7), core (8), right brick (9-15)
  ; Ball is 2 pixels wide - use ball center (ballx + 1) to determine side
  ; BRICKS PROTECT THE CORE - check bricks first, core only if brick is destroyed
  ; Sprite center is around X offset 8
  ; If ball center is left of center, check left brick first
  if ballx + 1 < player0x + 8 then goto __P1_Left_Brick
  ; Ball center is right of center, check right brick first
...
__P1_Left_Check_Core
  ...
  ; Ball missed core - try to hit right brick if intact
  if p1_bricks{1} then goto __P1_Right_Brick
  ; No brick to hit - pass through destroyed area
  return
...
__P1_Right_Check_Core
  ...
  ; If ball misses core, check if it can hit other intact bricks
  if ballx + 2 >= player0x + 5 then if ballx <= player0x + 11 then goto __P1_Core_Hit
  ; Ball missed core - try to hit left brick if intact
  if p1_bricks{2} then goto __P1_Left_Brick
  ; No brick to hit - pass through destroyed area
  return
...
__P1_Bottom_Check_Core
  ...
  ; If ball misses core, check if it can hit other intact bricks
  if bally + 4 >= player0y + 7 then if bally <= player0y + 20 then if ballx + 2 >= player0x + 5 then if ballx <= player0x + 11 then goto __P1_Core_Hit
  ; Ball missed core - try to hit top brick if intact
  if p1_bricks{0} then goto __P1_Top_Brick
  ; Try left/right bricks
  if p1_bricks{2} then goto __P1_Left_Brick
  if p1_bricks{1} then goto __P1_Right_Brick
  ; No brick to hit - pass through destroyed area
...
__P2_Top_Check_Core
  ...
  ; If ball misses core, check if it can hit other intact bricks
  if bally + 4 >= player1y + 7 then if bally <= player1y + 20 then if ballx + 2 >= player1x + 7 then if ballx <= player1x + 13 then goto __P2_Core_Hit
  ; Ball missed core - try to hit bottom brick if intact
  if p2_bricks{3} then goto __P2_Bottom_Brick
  ; Try left/right bricks
  if p2_bricks{2} then goto __P2_Left_Brick
  if p2_bricks{1} then goto __P2_Right_Brick
  ; No brick to hit - pass through destroyed area
  return
...
__P2_Middle_Brick
  ; Determine left vs core vs right based on X position
  ; NOTE: P2 hitboxes shifted +2 pixels right to align with sprite
  ; Sprite is ~16 pixels wide: left brick (0-9), core (10), right brick (11-17)
  ; Ball is 2 pixels wide - use ball center (ballx + 1) to determine side
  ; BRICKS PROTECT THE CORE - check bricks first, core only if brick is destroyed
  ; Sprite center is around X offset 10 (shifted +2 from P1's 8)
  ; If ball center is left of center, check left brick first
  if ballx + 1 < player1x + 10 then goto __P2_Left_Brick
  ; Ball center is right of center, check right brick first
...
__P2_Bottom_Check_Core
  ...
  if bally + 4 >= player1y + 7 then if bally <= player1y + 20 then if ballx + 2 >= player1x + 7 then if ballx <= player1x + 13 then goto __P2_Core_Hit
  ; Ball missed core - try to hit top brick if intact
  if p2_bricks{0} then goto __P2_Top_Brick
  ; Try left/right bricks
  if p2_bricks{2} then goto __P2_Left_Brick
  if p2_bricks{1} then goto __P2_Right_Brick
  ; No brick to hit - pass through destroyed area
