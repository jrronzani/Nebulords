  ;***************************************************************
  ;  NEBULORDS PXE - Version 085
  ;  Warlords-style Space Combat with Paddle Controls
  ;
  ;  Changes from v079:
  ;  - OPTIMIZE: Compress paddle position code using data tables
  ;  - REDUCE: ~1000 lines → ~100 lines (eliminates page boundary issues)
  ;  - REPLACE: 64 sprite definitions with 1 global paddle sprite
  ;  - REPLACE: on...goto with array indexing (no branch instructions)
  ;  - This optimization makes room for 4-player support and scoring
  ;
  ;  Changes from v077:
  ...
  ;***************************************************************
  ;  PADDLE POSITION DATA TABLES
  ;  X and Y offsets for all 32 paddle positions (relative to ship)
  ;***************************************************************
  data paddle_x_offsets
  ...
  ;***************************************************************
  ;  PADDLE SPRITE DEFINITION (used for all positions)
  ;***************************************************************
  player2:
    %011110
    %111111
...
__Award_P2_Point
  ...
  ;***************************************************************
__Update_P1_Paddle
  player2x = player0x + paddle_x_offsets[temp_dir]
  player2y = player0y + paddle_y_offsets[temp_dir]
  return

__Update_P2_Paddle
  player3x = player1x + paddle_x_offsets[temp_dir]
  player3y = player1y + paddle_y_offsets[temp_dir]
  return
