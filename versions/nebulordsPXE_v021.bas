  ;***************************************************************
  ;  NEBULORDS PXE - Version 021
  ;  Warlords-style Space Combat with Paddle Controls
  ;
  ;  Changes from v020:
  ;  - Implemented thrust-based physics (Asteroids-style momentum)
  ;  - Frame-based speed system: speed 2=fast, 10=slow/stopped
  ;  - Gradual acceleration/deceleration with accel_delay
  ;  - Ships drift with momentum when button released
  ;  - Wall bounce reverses direction
  ;  - 16-direction thrust based on paddle direction
  ;  - Physics adapted from "score and physics movement tests v013"
  ;
  ;  Physics System:
...
__P1_Accel_Right
  ...
  if p1_dir_x = 255 then p1_speed_x = p1_speed_x + 1 : if p1_speed_x >= 10 then p1_dir_x = 0 : p1_speed_x = 10
...
__P1_Accel_Left
  ...
  if p1_dir_x = 1 then p1_speed_x = p1_speed_x + 1 : if p1_speed_x >= 10 then p1_dir_x = 0 : p1_speed_x = 10
...
__P1_Accel_Up
  ...
  if p1_dir_y = 1 then p1_speed_y = p1_speed_y + 1 : if p1_speed_y >= 10 then p1_dir_y = 0 : p1_speed_y = 10
...
__P1_Accel_Down
  ...
  if p1_dir_y = 255 then p1_speed_y = p1_speed_y + 1 : if p1_speed_y >= 10 then p1_dir_y = 0 : p1_speed_y = 10
...
__P2_Accel_Right
  if p2_dir_x = 255 then p2_speed_x = p2_speed_x + 1 : if p2_speed_x >= 10 then p2_dir_x = 0 : p2_speed_x = 10
  if p2_dir_x = 0 then p2_dir_x = 1
  if p2_dir_x = 1 then if p2_speed_x > max_speed then p2_speed_x = p2_speed_x - 1
  return

__P2_Accel_Left
  if p2_dir_x = 1 then p2_speed_x = p2_speed_x + 1 : if p2_speed_x >= 10 then p2_dir_x = 0 : p2_speed_x = 10
  if p2_dir_x = 0 then p2_dir_x = 255
  if p2_dir_x = 255 then if p2_speed_x > max_speed then p2_speed_x = p2_speed_x - 1
  return

__P2_Accel_Up
  if p2_dir_y = 1 then p2_speed_y = p2_speed_y + 1 : if p2_speed_y >= 10 then p2_dir_y = 0 : p2_speed_y = 10
  if p2_dir_y = 0 then p2_dir_y = 255
  if p2_dir_y = 255 then if p2_speed_y > max_speed then p2_speed_y = p2_speed_y - 1
  return

__P2_Accel_Down
  if p2_dir_y = 255 then p2_speed_y = p2_speed_y + 1 : if p2_speed_y >= 10 then p2_dir_y = 0 : p2_speed_y = 10
