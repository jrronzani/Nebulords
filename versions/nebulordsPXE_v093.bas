  ...
  ; Player 3 (Paddle 2) - Physics (7 vars per player)
  dim p3_direction = var7
  dim p3_speed_x = var8          ; Frames between X moves
  dim p3_speed_y = var9          ; Frames between Y moves
  dim p3_frame_x = var10         ; Frame counter for X movement
  dim p3_frame_y = var11         ; Frame counter for Y movement
  dim p3_dir_x = var12           ; X direction
  dim p3_dir_y = var13           ; Y direction

  ; Player 4 (Paddle 3) - Physics (7 vars per player)
  dim p4_direction = var14
  dim p4_speed_x = var15         ; Frames between X moves
  dim p4_speed_y = var16         ; Frames between Y moves
  dim p4_frame_x = var17         ; Frame counter for X movement
  dim p4_frame_y = var18         ; Frame counter for Y movement
  dim p4_dir_x = var19           ; X direction
  dim p4_dir_y = var20           ; Y direction

  ; Shared
  dim accel_counter = p          ; Frame counter for acceleration delay
  dim game_mode = var21          ; 0=2P, 1=3P, 2=4P
  dim select_held = var22        ; Prevents multiple select triggers
  ...
  dim p3_bricks = var2           ; bit 0=top, bit 1=left, bit 2=right, bit 3=bottom (1=intact)
  dim p4_bricks = var3           ; bit 0=top, bit 1=left, bit 2=right, bit 3=bottom (1=intact)
  ...
  dim p3_score = var29           ; Player 3 score (00-99 in BCD)
  dim p4_score = var30           ; Player 4 score (00-99 in BCD)
  ...
  dim p3_caught_dir = var27      ; P3 paddle direction when ball caught
  dim p4_caught_dir = var28      ; P4 paddle direction when ball caught
  dim p3_catch_timer = var23     ; Countdown for auto-launch
  dim p4_catch_timer = var24     ; Countdown for auto-launch
  dim p3_state = var25           ; bit 0=button held, bit 1=cooldown active
  dim p4_state = var26           ; bit 0=button held, bit 1=cooldown active
  ; REPLACED - SEE BELOW
  ; REPLACED - SEE BELOW
  ; REPLACED - SEE BELOW
  ; REPLACED - SEE BELOW
...
__Game_Init
  ...
  ; P1/P2 positions (top row)
  player0x = 25 : player0y = 40
  player1x = 117 : player1y = 40

  ; P3/P4 positions (bottom row, 56 scanlines down)
  player4x = 25 : player4y = 96
  player5x = 117 : player5y = 96

  ; Initial paddle directions
  p1_direction = 24
  p2_direction = 8
  p3_direction = 0
  p4_direction = 16
  ...
  p1_score = $00 : p2_score = $00 : p3_score = $00 : p4_score = $00 : score_separator = $00
...
__Update_P2_Paddle
  ...
  ;***************************************************************
  ;  Update Player 3 Paddle Position (Paddle 2)
  ;  Same offsets as Player 1, same sprite
  ;***************************************************************
__Update_P3_Paddle
  player6:
  ...
  player6x = player4x + _paddle_x_offsets[p3_direction]
  player6y = player4y + _paddle_y_offsets[p3_direction]
  return



  ;***************************************************************
  ;  Update Player 4 Paddle Position (Paddle 3)
  ;  Same offsets as Player 1, same sprite
  ;***************************************************************
__Update_P4_Paddle
  player7:
  ...
  player7x = player5x + _paddle_x_offsets[p4_direction]
  player7y = player5y + _paddle_y_offsets[p4_direction]
  return
