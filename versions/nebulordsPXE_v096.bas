  ...
  ; REPLACED - SEE BELOW
  ;***************************************************************
  ;  SCORE SYSTEM - 4-Player Format (v096)
  ;  Format: score+2  score+1  score+0 = 6 BCD digits
  ;  Display: "_0000_" (blank, P1, P2, P3, P4, blank)
  ;
  ;  score_byte2 ($00): blank (upper nibble) + P1 score (lower nibble) = _0
  ;  score_byte1 ($00): P2 score (upper nibble) + P3 score (lower nibble) = 00
  ;  score_byte0 ($00): P4 score (upper nibble) + blank (lower nibble) = 0_
  ;***************************************************************
  dim score_byte2 = score+2      ; Blank + P1 score (0-9 BCD)
  dim score_byte1 = score+1      ; P2 score (upper) + P3 score (lower)
  dim score_byte0 = score         ; P4 score + blank
...
__Skip_Ball_Physics
  ...
  if joy0left && accel_counter = 0 then gosub __Load_P2_Context : temp_dir = p2_direction : gosub __Thrust : gosub __Store_P2_Context
...
__Round_Reset
  ...
  ;***************************************************************
  ;***************************************************************
  ;  SCORE SYSTEM - 4-Player Award Functions (v096)
  ;  Scores display automatically at top via built-in PXE!
  ;  Format: _0000_ (blank, P1, P2, P3, P4, blank)
  ;
  ;  P1 = Lower nibble of score_byte2
  ;  P2 = Upper nibble of score_byte1
  ;  P3 = Lower nibble of score_byte1 (not yet used)
  ;  P4 = Upper nibble of score_byte0 (not yet used)
  ;***************************************************************
__Award_P1_Point
  ; Increment P1 score (lower nibble of score_byte2, 0-9)
  temp1 = score_byte2 & $0F       ; Get P1 score (lower nibble)
  temp1 = temp1 + 1               ; Increment
  if temp1 > 9 then temp1 = 0     ; Wrap at 9
  score_byte2 = (score_byte2 & $F0) | temp1  ; Update P1 score, preserve blank
  return

__Award_P2_Point
  ; Increment P2 score (upper nibble of score_byte1, 0-9)
  temp1 = (score_byte1 & $F0)     ; Get P2 score (upper nibble)
  temp1 = temp1 + $10             ; Increment upper nibble
  if (temp1 & $F0) > $90 then temp1 = temp1 & $0F  ; Wrap at 9, preserve P3
