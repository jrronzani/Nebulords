   ;***************************************************************
   ;  PFSCORE TEST V11 - Zero-G Physics with Lower Max Speed
   ;
   ...
   ;
   ;  PHYSICS SYSTEM:
   ;  - speed_x/y: frames between pixel moves (10=slow, 3=FAST)
   ...
   ;  - MAX SPEED: 3 frames per pixel (slower than v10)
   ;
   ...
   dim speed_x = e                ; Frames between X moves (10=slow, 3=fast)
...
__Accel_Right
   ...
   if dir_x = 1 then if speed_x > 3 then speed_x = speed_x - 1
   return
...
__Accel_Left
   ...
   if dir_x = 255 then if speed_x > 3 then speed_x = speed_x - 1
   return
...
__Accel_Up
   ...
   if dir_y = 255 then if speed_y > 3 then speed_y = speed_y - 1
   return
...
__Accel_Down
   ...
   if dir_y = 1 then if speed_y > 3 then speed_y = speed_y - 1
   return
