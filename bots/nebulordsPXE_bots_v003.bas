  ;***************************************************************
  ;  NEBULORDS PXE - BOTS Version 003
  ;  Warlords-style Space Combat with Paddle Controls
  ;  EXPERIMENTAL: 1v1 AI Bot Mode (P1 Human vs P2 AI)
  ;
  ;  Based on: bots_v002 (Critical bug fixes)
  ;
  ;  Changes from bots_v002:
  ;  - FIX: Smooth rotation wraparound (no longer stuck at South!)
  ;  - FIX: AI can only thrust when aimed correctly (like humans)
  ;  - CHANGE: Use mask (& 31) for wraparound instead of conditionals
  ;  - CHANGE: Thrust only when within ±3 positions of target direction
  ;
  ;  Critical Fixes:
  ;  1. WRAPAROUND BUG: AI virtual paddle can now rotate freely through
  ;     position 0/31 barrier without getting stuck at South (75% down time)
  ;  2. THRUST DIRECTION BUG: AI can no longer thrust sideways while facing
  ;     wrong direction - must aim first, then thrust (like human players)
  ;
  ;  AI Behavior (HARD):
  ;  - Target updates every 30 frames (0.5 second reaction time)
  ;  - Aim noise: ±1 position for good accuracy
  ;  - Wander mode: 5% chance to ignore ball (rare)
  ;  - Thrust: Only when aimed within ±3 positions of target
  ;  - Catch: 50% chance when ball nearby (high success rate)
  ;
  ;  Changes from v096:
...
__AI_Update_P2
  ...
  ; 5% chance to wander (ignore ball) - HARD: less wandering
