  ;***************************************************************
  ;  NEBULORDS PXE - BOTS Version 016
  ...
  ;  - IMPROVE: Much harder AI (faster, more accurate, more aggressive)
  ;  - ADD: 3 tunable AI difficulty constants (easy to adjust)
  ;  - CHANGE: AI reaction time 15 frames (was 30) = 0.25s instead of 0.5s
  ;  - CHANGE: AI rotation speed 2 steps/frame (was 1) = twice as fast
  ;  - CHANGE: AI noise reduced to ±0 (was ±1) = perfect aiming
  ;  - CHANGE: AI wander reduced to 1% (was 5%) = more focused
  ;  - CHANGE: AI thrust chances increased to 50%/60% (was 25%/31%)
  ;  - CHANGE: AI catch chance increased to 100% (was 50%)
  ;  - CHANGE: AI aim tolerance tightened to ±2 (was ±3)
  ;
  ...
  ;***************************************************************
  ;  AI DIFFICULTY TUNING (EASY TO ADJUST!)
  ;  Change these 3 values to adjust bot difficulty
  ;***************************************************************
  const AI_REACTION_SPEED = 15   ; How fast AI recalculates target (frames, 60fps)
                                 ; EASY: 60 (1s) | MEDIUM: 30 (0.5s) | HARD: 15 (0.25s) | BRUTAL: 5 (0.08s)

  const AI_ACCURACY = 0          ; Aiming error (positions ±)
                                 ; EASY: 3 | MEDIUM: 1 | HARD: 0 (perfect)

  const AI_AGGRESSION = 3        ; How likely to thrust/catch (0-3)
                                 ; EASY: 0 (25% thrust, 50% catch)
                                 ; MEDIUM: 1 (31% thrust, 66% catch)
                                 ; HARD: 2 (44% thrust, 75% catch)
                                 ; BRUTAL: 3 (50%/60% thrust, 100% catch)
...
__Game_Init
  ...
  ; Initialize AI bot for Player 2 (uses AI_REACTION_SPEED constant)
  ai_target_direction = 16       ; Start facing south (toward center)
  ai_update_timer = AI_REACTION_SPEED  ; Uses tunable constant
...
__AI_Update_P2
  ...
  ; 1% chance to wander (ignore ball) - BRUTAL: minimal wandering
  if (rand & 127) < 1 then ai_target_direction = (rand & 31) : goto __AI_Rotate_P2
...
__AI_Calculate_Direction
  ...
  ; Add noise based on AI_ACCURACY (0=perfect, 1=±1, 2=±2, 3=±3)
  if AI_ACCURACY > 0 then temp1 = rand & AI_ACCURACY : if rand & 2 then temp1 = 0 - temp1
  if AI_ACCURACY > 0 then ai_target_direction = ai_target_direction + temp1
...
__AI_Rotate_P2
  ...
  ; Rotate 2 steps toward target (BRUTAL: twice as fast)
  if temp1 <= 16 then p2_direction = p2_direction + 2  ; Clockwise
  if temp1 > 16 then p2_direction = p2_direction - 2   ; Counter-clockwise
...
__AI_Actions_P2
  ...
  ; If aimed more than ±2 positions off target, NO THRUST/CATCH allowed (BRUTAL: tighter)
  if temp1 > 2 then p2_state{0} = 0 : return  ; Not aimed correctly, can't act
  ...
  ; Thrust decision based on AI_AGGRESSION (0=25%, 1=31%, 2=44%, 3=50%/60%)
  ; BRUTAL level 3: 50% when far, 60% when close
  if AI_AGGRESSION = 3 && temp1 > 40 && (rand & 15) < 8 then p2_state{0} = 1 : return   ; 50%
  if AI_AGGRESSION = 3 && temp1 < 15 && (rand & 15) < 10 then p2_state{0} = 1 : return  ; 60%
  if AI_AGGRESSION = 2 && temp1 > 40 && (rand & 15) < 7 then p2_state{0} = 1 : return   ; 44%
  if AI_AGGRESSION = 2 && temp1 < 15 && (rand & 15) < 7 then p2_state{0} = 1 : return   ; 44%
  if AI_AGGRESSION = 1 && temp1 > 40 && (rand & 15) < 5 then p2_state{0} = 1 : return   ; 31%
  if AI_AGGRESSION = 1 && temp1 < 15 && (rand & 15) < 5 then p2_state{0} = 1 : return   ; 31%
  if AI_AGGRESSION = 0 && temp1 > 40 && (rand & 15) < 4 then p2_state{0} = 1 : return   ; 25%
  if AI_AGGRESSION = 0 && temp1 < 15 && (rand & 15) < 4 then p2_state{0} = 1 : return   ; 25%

  ; Catch decision based on AI_AGGRESSION (0=50%, 1=66%, 2=75%, 3=100%)
  if ball_state = 0 && temp1 < 20 && AI_AGGRESSION = 3 then p2_state{0} = 1 : return  ; 100%
  if ball_state = 0 && temp1 < 20 && AI_AGGRESSION = 2 && (rand & 3) < 3 then p2_state{0} = 1 : return  ; 75%
  if ball_state = 0 && temp1 < 20 && AI_AGGRESSION = 1 && (rand & 2) < 2 then p2_state{0} = 1 : return  ; 66%
  if ball_state = 0 && temp1 < 20 && AI_AGGRESSION = 0 && (rand & 1) = 0 then p2_state{0} = 1 : return  ; 50%
