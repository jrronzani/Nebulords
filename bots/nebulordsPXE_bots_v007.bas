  ...
  ;  - FIX: P2 input now switches between AI and Paddle1 based on mode
  ;
  ;  Game Modes:
  ;  - Mode 0: 1 Player (P1 human) vs Bot (P2 AI)
  ;  - Mode 1: 2 Players (P1 and P2 both human)
  ;
  ;  Changes from bots_v005:
  ;  - MAJOR: Shared sprite data using player pointer variables
  ;  - OPTIMIZE: 6 sprite states defined once, both players reference same data
  ;  - OPTIMIZE: Reduced code from 3122 lines to 2467 lines (655 lines saved!)
  ;  - ROM SAVINGS: ~50% reduction in sprite definition space
  ;  - CHANGE: Last-man-standing scoring (1 point to survivor, not per kill)
  ;  - PREPARE: Scoring system now ready for 4-player expansion
  ;
  ;  Implementation based on Atari Age forum example:
  ;  - Uses player0pointerlo/hi and player1pointerlo/hi variables
  ;  - Address operators < and > to get low/high bytes of sprite data
  ;  - __Update_P1_Ship_Sprite and __Update_P2_Ship_Sprite now use pointers
  ;  - Both players share: _ship_state_0, 7, 11, 13, 14, 15
  ;
...
__Game_Init
  ...
  ;***************************************************************
  ;  SHARED SHIP SPRITE DATA - All 16 shield states defined ONCE
  ;  Both P1 and P2 will use pointers to reference these
  ;  26 scanlines tall, blocky design
  ;
  ;  State index = p1_bricks/p2_bricks (bits 0-3)
  ;  bit 0 = top, bit 1 = left, bit 2 = right, bit 3 = bottom
  ;***************************************************************

  ; State 15: All bricks intact (%1111)
  data _ship_state_15
  %00111100
  %00111100
  %00111100
  ...
  ; State 0: All bricks destroyed - core visible
  data _ship_state_0
  %00000000
  %00000000
  %00000000
  ...
  ;***************************************************************
  ;  SPRITE POINTER CONSTANTS - Memory addresses for each state
  ;  Defines both low and high bytes for all 16 shield states
  ;  States not yet fully defined reuse similar states (saves ROM)
  ;***************************************************************
  const ship_state_0_lo = <_ship_state_0
  const ship_state_0_hi = >_ship_state_0

  const ship_state_1_lo = <_ship_state_7
  const ship_state_1_hi = >_ship_state_7

  const ship_state_2_lo = <_ship_state_11
  const ship_state_2_hi = >_ship_state_11

  const ship_state_3_lo = <_ship_state_7
  const ship_state_3_hi = >_ship_state_7

  const ship_state_4_lo = <_ship_state_13
  const ship_state_4_hi = >_ship_state_13

  const ship_state_5_lo = <_ship_state_7
  const ship_state_5_hi = >_ship_state_7

  const ship_state_6_lo = <_ship_state_0
  const ship_state_6_hi = >_ship_state_0

  const ship_state_7_lo = <_ship_state_7
  const ship_state_7_hi = >_ship_state_7

  const ship_state_8_lo = <_ship_state_14
  const ship_state_8_hi = >_ship_state_14

  const ship_state_9_lo = <_ship_state_13
  const ship_state_9_hi = >_ship_state_13

  const ship_state_10_lo = <_ship_state_11
  const ship_state_10_hi = >_ship_state_11

  const ship_state_11_lo = <_ship_state_11
  const ship_state_11_hi = >_ship_state_11

  const ship_state_12_lo = <_ship_state_13
  const ship_state_12_hi = >_ship_state_13

  const ship_state_13_lo = <_ship_state_13
  const ship_state_13_hi = >_ship_state_13

  const ship_state_14_lo = <_ship_state_14
  const ship_state_14_hi = >_ship_state_14

  const ship_state_15_lo = <_ship_state_15
  const ship_state_15_hi = >_ship_state_15

  ;***************************************************************
  ;  Initial sprite setup for P0 (will be updated via pointers)
  ;***************************************************************
  player0:
  %00111100
  %00111100
  ...
  ;***************************************************************
  ;  Initial sprite setup for P1 (will be updated via pointers)
  ;***************************************************************
  player1:
  %00111100
  %00111100
...
__Main_Loop
  ...
  ; AI random launch: Launch after 2-4 seconds randomly
...
__Update_P1_Ship_Sprite
  ; Uses shared sprite data via player pointer variables
  ; p1_bricks contains 4 bits representing brick state (0-15)
  temp_dir = p1_bricks & %00001111
  on temp_dir goto __P1Spr_0 __P1Spr_1 __P1Spr_2 __P1Spr_3 __P1Spr_4 __P1Spr_5 __P1Spr_6 __P1Spr_7 __P1Spr_8 __P1Spr_9 __P1Spr_10 __P1Spr_11 __P1Spr_12 __P1Spr_13 __P1Spr_14 __P1Spr_15

__P1Spr_0
  player0pointerlo = ship_state_0_lo
  player0pointerhi = ship_state_0_hi
  return
__P1Spr_1
  player0pointerlo = ship_state_1_lo
  player0pointerhi = ship_state_1_hi
  return
__P1Spr_2
  player0pointerlo = ship_state_2_lo
  player0pointerhi = ship_state_2_hi
  return
__P1Spr_3
  player0pointerlo = ship_state_3_lo
  player0pointerhi = ship_state_3_hi
  return
__P1Spr_4
  player0pointerlo = ship_state_4_lo
  player0pointerhi = ship_state_4_hi
  return
__P1Spr_5
  player0pointerlo = ship_state_5_lo
  player0pointerhi = ship_state_5_hi
  return
__P1Spr_6
  player0pointerlo = ship_state_6_lo
  player0pointerhi = ship_state_6_hi
  return
__P1Spr_7
  player0pointerlo = ship_state_7_lo
  player0pointerhi = ship_state_7_hi
  return
__P1Spr_8
  player0pointerlo = ship_state_8_lo
  player0pointerhi = ship_state_8_hi
  return
__P1Spr_9
  player0pointerlo = ship_state_9_lo
  player0pointerhi = ship_state_9_hi
  return
__P1Spr_10
  player0pointerlo = ship_state_10_lo
  player0pointerhi = ship_state_10_hi
  return
__P1Spr_11
  player0pointerlo = ship_state_11_lo
  player0pointerhi = ship_state_11_hi
  return
__P1Spr_12
  player0pointerlo = ship_state_12_lo
  player0pointerhi = ship_state_12_hi
  return
__P1Spr_13
  player0pointerlo = ship_state_13_lo
  player0pointerhi = ship_state_13_hi
  return
__P1Spr_14
  player0pointerlo = ship_state_14_lo
  player0pointerhi = ship_state_14_hi
  return
__P1Spr_15
  player0pointerlo = ship_state_15_lo
  player0pointerhi = ship_state_15_hi
  return


__Update_P2_Ship_Sprite
  ; Uses shared sprite data via player pointer variables
  ; p2_bricks contains 4 bits representing brick state (0-15)
  temp_dir = p2_bricks & %00001111
  on temp_dir goto __P2Spr_0 __P2Spr_1 __P2Spr_2 __P2Spr_3 __P2Spr_4 __P2Spr_5 __P2Spr_6 __P2Spr_7 __P2Spr_8 __P2Spr_9 __P2Spr_10 __P2Spr_11 __P2Spr_12 __P2Spr_13 __P2Spr_14 __P2Spr_15

__P2Spr_0
  player1pointerlo = ship_state_0_lo
  player1pointerhi = ship_state_0_hi
  return
__P2Spr_1
  player1pointerlo = ship_state_1_lo
  player1pointerhi = ship_state_1_hi
  return
__P2Spr_2
  player1pointerlo = ship_state_2_lo
  player1pointerhi = ship_state_2_hi
  return
__P2Spr_3
  player1pointerlo = ship_state_3_lo
  player1pointerhi = ship_state_3_hi
  return
__P2Spr_4
  player1pointerlo = ship_state_4_lo
  player1pointerhi = ship_state_4_hi
  return
__P2Spr_5
  player1pointerlo = ship_state_5_lo
  player1pointerhi = ship_state_5_hi
  return
__P2Spr_6
  player1pointerlo = ship_state_6_lo
  player1pointerhi = ship_state_6_hi
  return
__P2Spr_7
  player1pointerlo = ship_state_7_lo
  player1pointerhi = ship_state_7_hi
  return
__P2Spr_8
  player1pointerlo = ship_state_8_lo
  player1pointerhi = ship_state_8_hi
  return
__P2Spr_9
  player1pointerlo = ship_state_9_lo
  player1pointerhi = ship_state_9_hi
  return
__P2Spr_10
  player1pointerlo = ship_state_10_lo
  player1pointerhi = ship_state_10_hi
  return
__P2Spr_11
  player1pointerlo = ship_state_11_lo
  player1pointerhi = ship_state_11_hi
  return
__P2Spr_12
  player1pointerlo = ship_state_12_lo
  player1pointerhi = ship_state_12_hi
  return
__P2Spr_13
  player1pointerlo = ship_state_13_lo
  player1pointerhi = ship_state_13_hi
  return
__P2Spr_14
  player1pointerlo = ship_state_14_lo
  player1pointerhi = ship_state_14_hi
  return
__P2Spr_15
  player1pointerlo = ship_state_15_lo
  player1pointerhi = ship_state_15_hi
  return
