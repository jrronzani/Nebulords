  ;***************************************************************
  ;  NEBULORDS PXE - BOTS Version 022
  ;  Warlords-style Space Combat with Paddle Controls
  ;  Supports 1P vs Bot and 2P vs 2P modes via Game Select switch
  ;
  ;  Based on: bots_v019
  ;
  ;  Changes from bots_v019:
  ;  - FIX: Tightened ship wall boundaries to prevent paddle sprite wrapping
  ;  - Ship boundaries: X:4-138 (was 3-139), Y:10-133 (was 8-135)
  ;  - Just 1 pixel tighter on sides, 2 pixels on top/bottom
  ;
...
__Wall_Bounce
  ...
  ; Check left wall (+1 pixel tighter)
  if temp_player_num = 0 then if player0x < 4 then temp_dir_x = 1 : player0x = 4 : temp_frame_x = temp_speed_x
  if temp_player_num = 1 then if player1x < 4 then temp_dir_x = 1 : player1x = 4 : temp_frame_x = temp_speed_x

  ; Check right wall (-1 pixel tighter)
  if temp_player_num = 0 then if player0x > 138 then temp_dir_x = 255 : player0x = 138 : temp_frame_x = temp_speed_x
  if temp_player_num = 1 then if player1x > 138 then temp_dir_x = 255 : player1x = 138 : temp_frame_x = temp_speed_x

  ; Check top wall (+2 pixels tighter)
  if temp_player_num = 0 then if player0y < 10 then temp_dir_y = 1 : player0y = 10 : temp_frame_y = temp_speed_y
  if temp_player_num = 1 then if player1y < 10 then temp_dir_y = 1 : player1y = 10 : temp_frame_y = temp_speed_y

  ; Check bottom wall (-2 pixels tighter)
  if temp_player_num = 0 then if player0y > 133 then temp_dir_y = 255 : player0y = 133 : temp_frame_y = temp_speed_y
  if temp_player_num = 1 then if player1y > 133 then temp_dir_y = 255 : player1y = 133 : temp_frame_y = temp_speed_y
