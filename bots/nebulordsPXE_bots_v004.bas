  ;***************************************************************
  ;  NEBULORDS PXE - BOTS Version 004
  ;  Warlords-style Space Combat with Paddle Controls
  ;  EXPERIMENTAL: 1v1 AI Bot Mode (P1 Human vs P2 AI)
  ;
  ;  Based on: bots_v003 (Intelligence upgrade: MEDIUM difficulty)
  ;
  ;  Changes from bots_v003 (Easy → Medium):
  ;  - IMPROVE: Reduce paddle flailing (wander 5%→1%, noise ±1→0)
  ;  - IMPROVE: Faster rotation (2 steps/frame instead of 1)
  ;  - IMPROVE: Smart shooting - aims at player's core when holding ball
  ;  - IMPROVE: Ball velocity awareness for better defense
  ;  - ADD: Strategic aiming mode when ball is caught
  ;  - ADD: Defensive positioning when opponent has ball
  ;
  ;  AI Behavior (MEDIUM):
  ;  - Target updates every 30 frames (0.5 second reaction time)
  ;  - Aim noise: NONE (perfect tracking)
  ;  - Wander mode: 1% chance (very focused)
  ;  - Rotation speed: 2 steps/frame (responsive)
  ;  - Smart launch: Aims at opponent's core before releasing ball
  ;  - Defense: Reads ball velocity, evades faster threats
  ;  - Thrust: Only when aimed within ±3 positions of target
  ;  - Catch: 50% chance when ball nearby
  ;
  ;  Changes from v096:
...
__P1_Button_Done
  ...
  ; Launch ball if caught and holding for auto-launch time
  if ball_state = 2 && p2_catch_timer >= auto_launch_time then gosub __P2_Launch_Ball
...
__AI_Update_P2
  ...
  ; 1% chance to wander (ignore ball) - MEDIUM: very focused
  if (rand & 127) < 1 then ai_target_direction = (rand & 31) : goto __AI_Rotate_P2

  ; SMART SHOOTING: If holding ball, aim at opponent's core (not ball!)
  if ball_state = 2 then temp1 = player0x - player1x : temp2 = player0y - player1y : goto __AI_Calculate_Direction

  ; Calculate direction to ball (simple 8-direction approximation)
...
__AI_Calculate_Direction
  ...
  ; Simple direction calculation (8 directions) - FIX: Correct paddle mapping!
  ; Paddle positions: 0=S, 8=W, 16=N, 24=E (bottom of dial = 0)
  ai_target_direction = 16  ; Default: North
  if ballx > player1x && bally > player1y then ai_target_direction = 28  ; SE (right+down)
  if ballx > player1x && bally < player1y then ai_target_direction = 20  ; NE (right+up)
  if ballx < player1x && bally > player1y then ai_target_direction = 4   ; SW (left+down)
  if ballx < player1x && bally < player1y then ai_target_direction = 12  ; NW (left+up)
  if ballx > player1x && temp1 > temp2 then ai_target_direction = 24     ; E (right)
  if ballx < player1x && temp1 > temp2 then ai_target_direction = 8      ; W (left)
  if bally > player1y && temp2 > temp1 then ai_target_direction = 0      ; S (down)
  if bally < player1y && temp2 > temp1 then ai_target_direction = 16     ; N (up)

  ; NO NOISE in v004 - perfect tracking for Medium difficulty
...
__AI_Rotate_P2
  ...
  ; Rotate 2 steps toward target (MEDIUM: faster response, less flailing)
...
__AI_Actions_P2
  ...
  ; VELOCITY AWARENESS: Check if ball moving fast toward AI
  ; ball_xvel positive = moving right, ball_yvel positive = moving down
  ; If ball moving toward AI quickly, prioritize evasion
  temp1 = ball_xvel
  if temp1 < 128 then temp1 = 0 - temp1  ; abs(ball_xvel)
  temp2 = ball_yvel
  if temp2 < 128 then temp2 = 0 - temp2  ; abs(ball_yvel)
  temp1 = temp1 + temp2  ; Total ball speed (Manhattan)

  ; If ball moving FAST (speed > 2) and headed toward AI, EVADE NOW!
  if ball_state = 0 && temp1 > 2 then p2_state{0} = 1 : return  ; Thrust away from fast ball
  ...
  ; Thrust decision: Chase if far (>40px), evade if very close (<15px) - MEDIUM: balanced
  ...
  ; Catch decision: 50% chance when ball nearby (<20px) - MEDIUM: good catching
