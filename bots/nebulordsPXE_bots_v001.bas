  ;***************************************************************
  ;  NEBULORDS PXE - BOTS Version 001
  ...
  ;
  ;  Changes from v098:
  ;  - ADD: AI bot system for Player 2
  ;  - ADD: Smooth paddle-like rotation toward target with delay
  ;  - ADD: Distance-based thrust and catch decision logic
  ;  - ADD: Reaction delay and aim noise for natural behavior
  ;  - CHANGE: P2 input uses AI instead of Paddle1
  ;  - CHANGE: 2-player mode (P3/P4 disabled)
  ;
  ;  AI Behavior:
  ;  - Target updates every 60 frames (1 second reaction time)
  ;  - Aim noise: ±2 positions for imperfect tracking
  ;  - Wander mode: 20% chance to ignore ball
  ;  - Thrust: Distance-based decisions (chase/evade)
  ;  - Catch: 30% chance when ball nearby
  ;
  ;  Changes from v096:
...
__Game_Init
  ...
  ai_update_timer = 60       ; First update in 1 second
  ai_action_timer = 30       ; First action check in 0.5 seconds
...
__AI_Update_P2
  ...
  ai_update_timer = 60  ; Reset to 1 second reaction time

  ; 20% chance to wander (ignore ball)
  if (rand & 7) < 2 then ai_target_direction = (rand & 31) : goto __AI_Rotate_P2
  ...
  ; Simple direction calculation (8 directions)
  ai_target_direction = 0   ; Default: North
  if ballx > player1x && bally > player1y then ai_target_direction = 4   ; SE
  if ballx > player1x && bally < player1y then ai_target_direction = 28  ; NE
  if ballx < player1x && bally > player1y then ai_target_direction = 12  ; SW
  if ballx < player1x && bally < player1y then ai_target_direction = 20  ; NW
  if ballx > player1x && temp1 > temp2 then ai_target_direction = 0      ; E
  if ballx < player1x && temp1 > temp2 then ai_target_direction = 16     ; W
  if bally > player1y && temp2 > temp1 then ai_target_direction = 8      ; S
  if bally < player1y && temp2 > temp1 then ai_target_direction = 24     ; N

  ; Add noise: ±2 positions for imperfect aim
  temp1 = (rand & 3) - 2  ; Random -2 to +1
...
__AI_Rotate_P2
  ...
  if p2_direction < 0 then p2_direction = 31
...
__AI_Actions_P2
  ...
  ; Thrust decision: Chase if far (>40px), evade if very close (<15px)
  if temp1 > 40 && (rand & 15) < 2 then p2_state{0} = 1 : return  ; 12% chance to thrust toward ball
  if temp1 < 15 && (rand & 15) < 3 then p2_state{0} = 1 : return  ; 18% chance to thrust (will evade)

  ; Catch decision: 30% chance when ball nearby (<20px)
  if ball_state = 0 && temp1 < 20 && (rand & 7) < 3 then p2_state{0} = 1 : return  ; Try to catch
