  ;***************************************************************
  ;  NEBULORDS PXE - BOTS Version 023
  ;  Warlords-style Space Combat with Paddle Controls
  ;  Supports 1P vs Bot and 2P vs 2P modes via Game Select switch
  ;
  ;  Based on: bots_v022
  ;
  ;  Changes from bots_v022:
  ;  - FIX: Tightened walls further to prevent ball sprite wrapping
  ;  - Ship boundaries: X:7-135 (was 4-138), Y:14-129 (was 10-133)
  ;  - Additional 3-4 pixels tighter on all sides
  ;
  ;  Changes from bots_v015:
  ;  - ADD: 3 tunable AI difficulty constants (minimal code bloat)
  ;  - KEEP: 1-step rotation (smooth), swatting noise, 5% wander (all good!)
  ;  - FIX: Use formulas instead of long conditionals (efficient for 4-player)
  ;
  ...
  dim round_reset_timer = var4 ; Countdown for invincibility
  ...
  ;***************************************************************
  dim death_anim_frame = var17    ; Current frame of death animation (0-15)
  dim death_anim_active = var18   ; 0=inactive, 1=active
  dim death_anim_timer = var19    ; Frame counter for animation speed
  ; REPLACED - SEE BELOW
  ; REPLACED - SEE BELOW
  ; REPLACED - SEE BELOW
  ; REPLACED - SEE BELOW
...
__Game_Init
  ...
  round_reset_timer = 0    ; No invincibility at start
  ...
  ; Initialize AI bot for Player 2
  ai_target_direction = 16       ; Start facing south (toward center)
  ai_update_timer = AI_REACTION  ; Uses tunable constant
  ai_action_timer = 15       ; First action check in 0.25 seconds

  ; Initialize death animation
  death_anim_active = 0      ; No animation at start
  death_anim_frame = 0
  death_anim_timer = 0
  player8y = 200             ; Hide death animation sprite off-screen
  ...
  ;***************************************************************
  ;  Death Animation Sprite - Frame 0 (initial sprite definition)
  ;  16 frames total, switched dynamically in __Update_Death_Animation
  ;***************************************************************
  player8:
  %00000000
  %00000000
...
__P1_Button_Done
  ...
  ; DISABLED: if round_reset_timer > 0 then goto __Skip_Brick_Collision
...
__Skip_Ball_Physics
  ...
  if round_reset_timer > 0 then round_reset_timer = round_reset_timer - 1
  if round_reset_timer = 1 then gosub __Round_Reset
  ...
  ;  Advances animation frame every 3 game frames (~20fps animation)
  ;***************************************************************
  if death_anim_active then gosub __Advance_Death_Animation
...
__Wall_Bounce
  ...
  ; Check left wall
  if temp_player_num = 0 then if player0x < 7 then temp_dir_x = 1 : player0x = 7 : temp_frame_x = temp_speed_x
  if temp_player_num = 1 then if player1x < 7 then temp_dir_x = 1 : player1x = 7 : temp_frame_x = temp_speed_x

  ; Check right wall
  if temp_player_num = 0 then if player0x > 135 then temp_dir_x = 255 : player0x = 135 : temp_frame_x = temp_speed_x
  if temp_player_num = 1 then if player1x > 135 then temp_dir_x = 255 : player1x = 135 : temp_frame_x = temp_speed_x

  ; Check top wall
  if temp_player_num = 0 then if player0y < 14 then temp_dir_y = 1 : player0y = 14 : temp_frame_y = temp_speed_y
  if temp_player_num = 1 then if player1y < 14 then temp_dir_y = 1 : player1y = 14 : temp_frame_y = temp_speed_y

  ; Check bottom wall
  if temp_player_num = 0 then if player0y > 129 then temp_dir_y = 255 : player0y = 129 : temp_frame_y = temp_speed_y
  if temp_player_num = 1 then if player1y > 129 then temp_dir_y = 255 : player1y = 129 : temp_frame_y = temp_speed_y
...
__P1_Core_Hit
  ...
  ; Start death animation at core position (+4x, +12y from ship origin)
  player8x = player0x + 4
  player8y = player0y + 12
  death_anim_frame = 0
  death_anim_timer = 0
  death_anim_active = 1
...
__Award_P2_Point
  ...
  ;  Counts alive players (Y < 150 = on screen) and awards point to survivor
  ;  Scalable for 4-player mode
  ;***************************************************************
__Check_Last_Man_Standing
  ; Count alive players (Y < 150 = playfield bottom, Y >= 150 = dead/off-screen)
  temp1 = 0
  if player0y < 150 then temp1 = temp1 + 1
  if player1y < 150 then temp1 = temp1 + 1
  ...
  if player0y < 150 then gosub __Award_P1_Point
  if player1y < 150 then gosub __Award_P2_Point
...
__AI_Calculate_Direction
  ...
  ; Add swatting noise (tunable: 0=perfect, 1=±1, 2=±2, 3=±3)
  if AI_SWAT > 0 then temp1 = rand & AI_SWAT : if rand & 2 then temp1 = 0 - temp1 : ai_target_direction = ai_target_direction + temp1
...
__AI_Actions_P2
  ...
  ; Thrust decision (tunable: AGGRO=0:25%, 1:37%, 2:50%, 3:62%)
  if temp1 > 40 && (rand & 15) < (4 + AI_AGGRO * 2) then p2_state{0} = 1 : return
  if temp1 < 15 && (rand & 15) < (5 + AI_AGGRO * 2) then p2_state{0} = 1 : return

  ; Catch decision (tunable: AGGRO=0:50%, 1:66%, 2:75%, 3:100%)
  if ball_state = 0 && temp1 < 20 && (rand & 3) <= AI_AGGRO then p2_state{0} = 1 : return
...
__Setup_Playfield
  ...
  ;***************************************************************
  ;  Death Animation Subroutines
  ;***************************************************************

__Advance_Death_Animation
  ; Increment animation timer
  death_anim_timer = death_anim_timer + 1
  ; Advance frame every 3 game ticks (~20fps animation at 60fps game)
  if death_anim_timer < 3 then return
  death_anim_timer = 0
  ; Advance to next frame
  death_anim_frame = death_anim_frame + 1
  ; Check if animation complete (16 frames total)
  if death_anim_frame >= 16 then death_anim_active = 0 : player8y = 200 : return
  ; Update sprite to new frame
  gosub __Update_Death_Animation
  return

__Update_Death_Animation
  ; Dispatch to appropriate frame based on death_anim_frame (0-15)
  on death_anim_frame goto __DA_0 __DA_1 __DA_2 __DA_3 __DA_4 __DA_5 __DA_6 __DA_7 __DA_8 __DA_9 __DA_10 __DA_11 __DA_12 __DA_13 __DA_14 __DA_15
...
__DA_0
  ...
  %00000000,%00000100,%00100000,%00000100,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  ...
  $8C,$8C,$8C,$8C,$8C,$00,$9C,$92,$CA,$38,$66,$A4,$00,$8C,$8C,$8C
  $8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C
  $8C,$8C,$8C
end
  return
...
__DA_1
  ...
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00001000,%00111000,%11111100
  %11111100,%00111111,%00111111,%00011100,%00010000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000
end
  player8color:
  $8C,$96,$98,$92,$92,$92,$90,$92,$8C,$98,$90,$90,$00,$00,$00,$96
  $96,$00,$A0,$A0,$0E,$66,$68,$6A,$6C,$6C,$6A,$68,$66,$0E,$00,$A0
  $00,$96,$96,$00,$00,$00,$90,$92,$92,$8C,$92,$90,$92,$92,$92,$92
  $94,$A0
end
  return
...
__DA_2
  ...
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00001000,%00001000,%00011000,%00011000,%00011000,%00011000,%00011000,%01111000
  %11111110,%01111111,%00011110,%00011000,%00011000,%00011000,%00011000,%00011000
  ...
  player8color:
  $8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C
  $76,$66,$66,$66,$66,$66,$68,$6A,$6C,$4E,$6C,$68,$66,$66,$66,$66
  $66,$86,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C
  $8C,$8C
end
  return
...
__DA_3
  player8:
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00001000,%00001000,%00001000,%00001000
  %00011000,%00011000,%00011000,%00011000,%00011000,%00011000,%00011000,%01111000
  %01111100,%00111110,%00011110,%00011000,%00011000,%00011000,%00011000,%00011000
  %00011000,%00011000,%00010000,%00010000,%00010000,%00010000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000
end
  player8color:
  $8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$86,$84,$74,$74
  $66,$66,$66,$66,$66,$68,$6A,$6C,$7E,$7E,$6C,$6A,$68,$66,$66,$66
  $66,$66,$74,$74,$84,$86,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C
  $8C,$8C
end
  return
...
__DA_4
  ...
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00001000,%00001000,%00001000,%00001000
  %00001000,%00011000,%00011000,%00001000,%00001000,%00001000,%00001000,%00111000
  %00111100,%00111100,%00011100,%00010000,%00010000,%00010000,%00010000,%00011000
  %00011000,%00010000,%00010000,%00010000,%00010000,%00010000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000
end
  player8color:
  $8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$84,$84,$74,$66
  $66,$66,$66,$66,$66,$68,$6A,$4E,$0E,$0E,$7E,$6A,$68,$66,$66,$66
  $66,$66,$74,$84,$84,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C
  $8C,$8C
end
  return
...
__DA_5
  player8:
  %00000000,%00000000,%00000000,%00000000,%00000000,%00001000,%00011000,%00111000
  %00111000,%00100100,%00000100,%00001000,%00011000,%00011000,%00011100,%00001100
  %00001100,%00001010,%00001100,%00001100,%00011100,%00111100,%00111100,%00111100
  %01111100,%01111110,%00111110,%00111110,%00111100,%00111000,%00110000,%00110000
  %00010000,%00110000,%00110000,%00111000,%00011000,%00011000,%00010000,%00100000
  %00100100,%00011100,%00011100,%00011000,%00010000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000
end
  player8color:
  $8C,$8C,$8C,$8C,$8C,$BA,$BA,$AA,$AA,$98,$96,$84,$74,$74,$66,$66
  $66,$66,$66,$68,$68,$6C,$7E,$0E,$0E,$8E,$0E,$7E,$6C,$68,$68,$66
  $66,$66,$66,$66,$74,$74,$84,$96,$98,$AA,$AA,$BA,$BA,$8C,$8C,$8C
  $8C,$8C
end
  return
...
__DA_6
  player8:
  %00000000,%00001000,%00011000,%00001000,%00001000,%00001000,%00001000,%00001000
  %00001000,%00000000,%00001000,%00001000,%00001000,%00001000,%00001000,%00000000
  %00001000,%00001000,%00011000,%00011000,%00011000,%00011000,%00011100,%00111100
  %00111100,%00111100,%00111100,%00111000,%00011000,%00011000,%00011000,%00011000
  %00010000,%00010000,%00000000,%00010000,%00010000,%00010000,%00010000,%00010000
  %00000000,%00010000,%00010000,%00010000,%00010000,%00010000,%00010000,%00011000
  %00010000,%00000000,%00000000
end
  player8color:
  $8C,$BC,$BA,$BA,$BA,$BA,$BA,$AA,$AA,$8C,$98,$84,$84,$74,$66,$8C
  $66,$66,$66,$66,$68,$6A,$6C,$0E,$0E,$0E,$0E,$5E,$6A,$68,$66,$66
  $66,$66,$8C,$66,$74,$84,$84,$98,$8C,$AA,$AA,$BA,$BA,$BA,$BA,$BA
  $BC,$8C
end
  return
...
__DA_8
  player8:
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%01000000,%01000000,%00100000,%00111110,%00011110,%00000110
  %00001100,%00001100,%00011110,%00000010,%00000000,%01010000,%00000000,%00000100
  %00000010,%01000000,%00100000,%00000000,%00001010,%00000000,%01000000,%01111000
  %00110000,%00110000,%01100000,%01111000,%00111100,%00000100,%00000010,%00000010
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000
end
  player8color:
  $8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$86,$84,$74,$66,$66,$66
  $66,$66,$66,$66,$8C,$6A,$8C,$7E,$8C,$8C,$4E,$8C,$6A,$8C,$68,$66
  $66,$66,$66,$66,$66,$66,$74,$84,$86,$8C,$8C,$8C,$8C,$8C,$8C,$8C
  $8C,$8C
end
  return
...
__DA_9
  player8:
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000110,%00011100,%00111110
  %00000001,%01000000,%11000000,%00000000,%00000100,%00001100,%00001000,%00111000
  %00111001,%10011100,%00011100,%00010000,%00110000,%00100000,%00000000,%00000000
  %10000010,%10000000,%01111100,%00111000,%01100000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000
end
  player8color:
  $8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$74,$66,$66
  $66,$66,$56,$8C,$6A,$7E,$0E,$0E,$BE,$BE,$0E,$0E,$7E,$6A,$8C,$8C
  $5A,$66,$66,$66,$66,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C
  $8C,$8C
end
  return
...
__DA_10
  player8:
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00001111,%00000110,%00001110
  %00011110,%00011010,%00000001,%01000001,%00000001,%01000010,%00110100,%00000100
  %00000100,%00100000,%00100000,%00101100,%00000010,%01000000,%00000000,%10000000
  %11011001,%01111000,%01110000,%01000000,%01110000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000
end
  player8color:
  $8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$66,$66,$66
  $66,$66,$68,$68,$6C,$8C,$0E,$8E,$BE,$BC,$8E,$0E,$8C,$8C,$8C,$64
  $66,$66,$66,$66,$66,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C
  $8C,$8C
end
  return
...
__DA_11
  player8:
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00011110,%00000000,%01000000
  %01000000,%00000000,%00010000,%00000000,%00001000,%00011000,%01111000,%01101100
  %01111010,%01010110,%00111110,%00011100,%00011000,%00010000,%00000000,%00001000
  %00000000,%10000010,%10000010,%00000000,%01111000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000
end
  player8color:
  $8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$66,$8C,$66
  $66,$8C,$68,$8C,$5E,$0E,$0E,$BE,$BE,$BC,$BE,$0E,$0E,$5E,$8C,$68
  $8C,$66,$66,$8C,$66,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C
  $8C,$8C
end
  return
...
__DA_12
  ...
  %00000000,%00010000,%00000000,%00001100,%00011100,%00111000,%01111100,%01111100
  %01100110,%01100110,%00111110,%00111110,%00011100,%00111000,%00110000,%00000000
  %00001000,%00000000,%00000000,%00000010,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000
end
  player8color:
  $8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$66,$8C
  $8C,$66,$8C,$6C,$7E,$0E,$8E,$BE,$BC,$BC,$BE,$8E,$0E,$7E,$6C,$8C
  $66,$8C,$8C,$66,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C
  $8C,$8C
end
  return
...
__DA_13
  ...
  %00000110,%00000010,%00000100,%00011110,%00011110,%00000000,%01000000,%00000000
  %00010000,%00010000,%00000000,%00001100,%00011100,%00111100,%11111100,%01111110
  %01010110,%01101010,%01111110,%00111111,%00111100,%00111000,%00110000,%00000000
  %00001000,%00001000,%00000000,%00000010,%00000000,%01111000,%01111000,%00100000
  %01000000,%01100000,%00100000,%00000000,%00000000,%00000000,%00000000,%00000000
  %00000000,%00000000,%00000000
end
  player8color:
  $8C,$8C,$8C,$8C,$8C,$8C,$8C,$8C,$96,$84,$84,$74,$66,$8C,$66,$8C
  $66,$66,$8C,$6A,$7E,$0E,$BE,$BE,$BC,$BC,$BE,$BE,$0E,$7E,$6A,$8C
  $66,$66,$8C,$66,$8C,$66,$74,$84,$84,$96,$8C,$8C,$8C,$8C,$8C,$8C
  $8C,$8C
end
  return
...
__DA_14
  player8:
  %00000000,%00000000,%00000000,%00000000,%01100000,%00011111,%00000101,%00000010
  %00000100,%00011110,%00011011,%00000011,%00000001,%00011000,%00010000,%00000000
  %00000000,%00000100,%00011110,%00111100,%01110100,%11010110,%11011010,%01101110
  %00000111,%11100000,%01110110,%01011011,%01101011,%00101110,%00111100,%00011000
  %00100000,%00000000,%00000000,%00001000,%00011000,%00001001,%11000010,%11011000
  %01111000,%00100000,%01000000,%10100000,%01111000,%00000110,%00000000,%00000000
  %00000000,%00000000,%00000000
end
  player8color:
  $8C,$8C,$8C,$8C,$BA,$BA,$AA,$98,$96,$84,$84,$74,$74,$66,$66,$8C
  $8C,$68,$68,$6C,$7E,$0E,$8E,$BC,$BC,$BA,$BC,$8E,$0E,$7E,$6C,$6A
  $68,$8C,$8C,$66,$66,$66,$74,$84,$84,$96,$98,$AA,$BA,$BA,$8C,$8C
  $8C,$8C
end
  return
...
__DA_15
  ...
  %00000000,%00000000,%00000000,%00000000,%01100000,%00011111,%00000101,%00000010
  %00000100,%00011110,%00011011,%00000011,%00000001,%00011000,%00010000,%00000000
  %00000000,%00000100,%00011110,%00111100,%01110100,%11010110,%11011010,%01111110
  %00000111,%11100000,%01111110,%01011011,%01101011,%00101110,%00111100,%00011000
  %00100000,%00000000,%00000000,%00001000,%00011000,%00001001,%11000010,%11011000
  %01111000,%00100000,%01000000,%10100000,%01111000,%00000110,%00000000,%00000000
  %00000000,%00000000,%00000000
end
