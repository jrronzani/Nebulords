  ;***************************************************************
  ;  NEBULORDS PXE - BOTS Version 021
  ;  Warlords-style Space Combat with Paddle Controls
  ;  Supports 1P vs Bot and 2P vs 2P modes via Game Select switch
  ;
  ;  Based on: bots_v020
  ;
  ;  Changes from bots_v020:
  ;  - FIX: Reduced paddle wall constraints (less restrictive, more playable)
  ;  - FIX: Added alive checks - dead players' paddles now hide (Y=200)
  ;  - Paddles now X:8-132, Y:24-118 (was 11-129, 26-117)
  ;
...
__Update_P2_Paddle
  ...
  ;***************************************************************
  ;  Paddle Wall Boundaries - Prevent ball sprite wrapping
  ;  Ball max offsets: X-7 to X+26, Y-18 to Y+40
  ;  Less restrictive: allows paddles closer to edges (2-3 pixel buffer)
  ;  Also hides paddles when player is dead (Y >= 150)
  ;***************************************************************
__Paddle_Walls
  ; P1 paddle - hide if dead, otherwise constrain to walls
  if player0y >= 150 then player2y = 200 : return
  if player2x < 8 then player2x = 8
  if player2x > 132 then player2x = 132
  if player2y < 24 then player2y = 24
  if player2y > 118 then player2y = 118
  ; P2 paddle - hide if dead, otherwise constrain to walls
  if player1y >= 150 then player3y = 200 : return
  if player3x < 8 then player3x = 8
  if player3x > 132 then player3x = 132
  if player3y < 24 then player3y = 24
  if player3y > 118 then player3y = 118
  return
