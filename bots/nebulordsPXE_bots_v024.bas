  ;***************************************************************
  ;  NEBULORDS PXE - BOTS Version 024
  ;  Warlords-style Space Combat with Paddle Controls
  ;  Supports 1P vs Bot and 2P vs 2P modes via Game Select switch
  ;
  ;  Based on: bots_v023
  ;
  ;  Changes from bots_v023:
  ;  - ADD: Death animation when player core is hit
  ;  - Uses player8 sprite (8px wide, 50px tall, 15 frames)
  ;  - Animation positioned 4px right and 13px lower than ship sprite
  ;  - Same animation used for all player deaths
  ;
  ...
  ;***************************************************************
  dim death_anim_frame = var17   ; Animation frame counter (0-14 = playing, 255 = inactive)
...
__Game_Init
  ...
  death_anim_frame = 255     ; No animation playing
  player8y = 200              ; Hide player8 sprite off-screen
...
__Skip_Ball_Physics
  ...
  ;  Plays 15-frame explosion animation when player core is hit
  ;***************************************************************
  if death_anim_frame < 15 then gosub __Play_Death_Animation
...
__P1_Core_Hit
  ; Player 1 dies - start death animation
  ; Position death animation sprite (4px right, 13px lower than ship)
  player8x = player0x + 4
  player8y = player0y + 13
  death_anim_frame = 0       ; Start animation from frame 0
...
__P2_Core_Hit
  ; Player 2 dies - start death animation
  ; Position death animation sprite (4px right, 13px lower than ship)
  player8x = player1x + 4
  player8y = player1y + 13
  death_anim_frame = 0       ; Start animation from frame 0
...
__Check_Last_Man_Standing
  ...
  ;***************************************************************
  ;  Death Animation Playback Subroutine
  ;  Advances animation frame and hides sprite when complete
  ;***************************************************************
__Play_Death_Animation
  ; Set player8 sprite pointer to current animation frame
  ; Each frame is 50 bytes (50 scanlines), so offset = frame * 50
  temp1 = death_anim_frame * 50
  player8pointer = player8 + temp1

  ; Increment animation frame counter for next time
  death_anim_frame = death_anim_frame + 1

  ; Check if animation is complete (15 frames total)
  if death_anim_frame >= 15 then player8y = 200 : death_anim_frame = 255
...
; Digit 5 (P2 ones - Purple gradient)
  ...
  ;#######################################################################
  ;
  ;  SECTION: DEATH ANIMATION SPRITE DATA
  ;  15 frames, 50 pixels tall each
  ;  Animation plays when player core is hit
  ;
  ;#######################################################################
  ;#######################################################################

; Frame 0 - Initial explosion spark
  player8:
  %00000000
  %00000000
  %00000000
