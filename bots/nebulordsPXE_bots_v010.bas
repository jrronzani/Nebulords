  ;***************************************************************
  ;  NEBULORDS PXE - BOTS Version 009
  ;  Warlords-style Space Combat with Paddle Controls
  ;  4-PLAYER MODE: 1 Human vs 3 AI Bots
  ;
  ;  Based on: bots_v008 (Fix game logic and P3/P4 functionality)
  ;
  ;  Changes from bots_v008:
  ;  - FIX: Winning condition - check if only 1 ship alive before awarding points
  ;  - FIX: Round restart - only restart when round ends (last ship standing)
  ;  - FIX: P3/P4 thrust physics
  ;  - FIX: P2 AI behavior (debug coordinate tracking)
  ;  - FIX: Round reset to respawn all 4 ships with full bricks
  ;  - FIX: Add full sprite frames for P3/P4 (states 1-14)
  ;  - ADD: Player-to-player collision for P3/P4
  ;***************************************************************
...
__Game_Init
  ...
  ; Single-width for paddles (player4-7)
  NUSIZ4 = $00   ; P1 paddle
  NUSIZ5 = $00   ; P2 paddle
  NUSIZ6 = $00   ; P3 paddle
  NUSIZ7 = $00   ; P4 paddle
  ...
  player2x = 25 : player2y = 106   ; P3 ship: Bottom-left (orange) - moved up 30px
  player3x = 117 : player3y = 106  ; P4 ship: Bottom-right (green) - moved up 30px
  ...
  ; Paddle colors - P3 paddle (orange, same as P3 ship)
  player6color:
  $20
  $22
  $24
  ...
  ; Paddle colors - P4 paddle (green, same as P4 ship)
  player7color:
  $C0
  $C2
  $C4
...
__Main_Loop
  ...
  ;  Only update AI if player is alive (y < 200)
  ;***************************************************************
  if player1y < 200 then gosub __AI_Update_P2  ; AI calculates direction and actions for P2
  if player2y < 200 then gosub __AI_Update_P3  ; AI calculates direction and actions for P3
  if player3y < 200 then gosub __AI_Update_P4  ; AI calculates direction and actions for P4
...
__Skip_Ball_Physics
  ...
  ; Thrust Physics - Apply when button held (only if alive)
  ; Player 1: Paddle 0 button is joy0right
  if player0y < 200 then if joy0right && accel_counter = 0 then gosub __Load_P1_Context : temp_dir = p1_direction : gosub __Thrust : gosub __Store_P1_Context

  ; Player 2: AI bot sets p2_state{0} for thrust decisions
  if player1y < 200 then if p2_state{0} && accel_counter = 0 then gosub __Load_P2_Context : temp_dir = p2_direction : gosub __Thrust : gosub __Store_P2_Context

  ; Player 3: AI bot sets p3_state{0} for thrust decisions
  if player2y < 200 then if p3_state{0} && accel_counter = 0 then gosub __Load_P3_Context : temp_dir = p3_direction : gosub __Thrust : gosub __Store_P3_Context

  ; Player 4: AI bot sets p4_state{0} for thrust decisions
  if player3y < 200 then if p4_state{0} && accel_counter = 0 then gosub __Load_P4_Context : temp_dir = p4_direction : gosub __Thrust : gosub __Store_P4_Context

  ; Apply frame-based movement (drift with momentum) - only if alive
  if player0y < 200 then gosub __Load_P1_Context : gosub __Apply_Movement : gosub __Store_P1_Context
  if player1y < 200 then gosub __Load_P2_Context : gosub __Apply_Movement : gosub __Store_P2_Context
  if player2y < 200 then gosub __Load_P3_Context : gosub __Apply_Movement : gosub __Store_P3_Context
  if player3y < 200 then gosub __Load_P4_Context : gosub __Apply_Movement : gosub __Store_P4_Context

  ;***************************************************************
  ;  Wall Bounce for Players - only if alive
  ;***************************************************************
  if player0y < 200 then gosub __Load_P1_Context : gosub __Wall_Bounce : gosub __Store_P1_Context
  if player1y < 200 then gosub __Load_P2_Context : gosub __Wall_Bounce : gosub __Store_P2_Context
  if player2y < 200 then gosub __Load_P3_Context : gosub __Wall_Bounce : gosub __Store_P3_Context
  if player3y < 200 then gosub __Load_P4_Context : gosub __Wall_Bounce : gosub __Store_P4_Context
...
__P1_Core_Hit
  ...
  player4y = 200
  ; Check if round is over (only 1 ship remaining)
  gosub __Check_Round_Over
  return

  ;***************************************************************
  ;  Check if Round is Over (only 1 ship remaining)
  ;  Awards point to survivor and starts round reset
  ;***************************************************************
__Check_Round_Over
  ; Count alive ships (y < 200 means visible/alive)
  temp1 = 0  ; Alive count
  if player0y < 200 then temp1 = temp1 + 1
  if player1y < 200 then temp1 = temp1 + 1
  if player2y < 200 then temp1 = temp1 + 1
  if player3y < 200 then temp1 = temp1 + 1

  ; If more than 1 ship alive, continue playing
  if temp1 > 1 then return

  ; Only 1 ship left - determine winner and award point
  if player0y < 200 then gosub __Award_P1_Point : goto __Start_Round_Reset
  if player1y < 200 then gosub __Award_P2_Point : goto __Start_Round_Reset
  if player2y < 200 then gosub __Award_P3_Point : goto __Start_Round_Reset
  if player3y < 200 then gosub __Award_P4_Point : goto __Start_Round_Reset
...
__Award_P3_Point
  ; Increment P3 score (lower nibble of score_byte1, CAP at 9)
  temp1 = score_byte1 & $0F       ; Get P3 score (lower nibble)
  if temp1 < 9 then temp1 = temp1 + 1  ; Increment only if less than 9
  score_byte1 = (score_byte1 & $F0) | temp1  ; Update P3 score, preserve P2 score
  return

__Award_P4_Point
  ; Increment P4 score (upper nibble of score_byte0, CAP at 9)
  temp1 = (score_byte0 & $F0)     ; Get P4 score (upper nibble)
  if (temp1 & $F0) < $90 then temp1 = temp1 + $10  ; Increment only if less than 9
  score_byte0 = (score_byte0 & $0F) | temp1  ; Update P4 score, preserve blank
  return
...
__Update_P2_Paddle
  ...
  ;***************************************************************
  ;  Update Player 3 Paddle Position
  ;  Same sprite as P1/P2, orange color
  ;***************************************************************
__Update_P3_Paddle
  player6:
  %011110
  ...
  player6x = player2x + _paddle_x_offsets[p3_direction]
  player6y = player2y + _paddle_y_offsets[p3_direction]
  return



  ;***************************************************************
  ;  Update Player 4 Paddle Position
  ;  Same sprite as P1/P2, green color
  ;***************************************************************
__Update_P4_Paddle
  player7:
  %011110
  ...
  player7x = player3x + _paddle_x_offsets[p4_direction]
  player7y = player3y + _paddle_y_offsets[p4_direction]
  return
