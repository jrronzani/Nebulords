   ...
   ;  Version 50 - Fix side OOB detection and improve warning flash
   ;  Based on v49 (OOB with caught balls)
   ;
   ;  P2 starting position set to 108 for visual symmetry
   ;  Ball respawns at top center if out of bounds for 1 second
   ;  Ball flashes 6 on/2 off for better visibility
   ;  OOB boundaries moved further inside to catch stuck balls
   ;  OOB check runs before snap-back to fix left/right detection
   ;
...
__Check_OOB
   ...
   ; Aggressive boundaries to catch balls stuck in thick barriers
   ; Top/bottom moved further in to catch balls in thick playfield walls
   if ballx < 15 then goto __Ball_OOB
   if ballx > 145 then goto __Ball_OOB
...
__Warning_Flash
   ...
   ; Flash ball by alternating ballheight (6 frames on, 2 frames off)
   temp1 = ball_warning & 7  ; Get 0-7 (cycles every 8 frames)
   if temp1 < 6 then ballheight = 1 else ballheight = 0
   return
