   ...
   ;  Version 40 - Edge-based collision lookahead for wall-sliding
   ;  Based on v36 (stable milestone)
   ;
   ;  Positions missiles at sprite EDGES (not paddle) to test collision
   ;  - When moving right: check right edge of sprite
   ;  - When moving down: check bottom edge of sprite
   ;  Independent X/Y axis checking enables smooth diagonal wall-sliding
   ;
...
__Main_Loop
   ...
   ;***************************************************************
   ;  Handle Player 1 controls - Edge-based collision lookahead
   ;  Sprite: 8px wide (player0x to player0x+8), 13px tall (player0y-13 to player0y)
   ;  X and Y checked independently for diagonal wall-sliding
   ;***************************************************************
   ; Only process if alive
   if !p1_alive then goto __Skip_P1_Controls

   ; X axis movement: test at sprite's left/right edge
   p1_xspeed = 0
   if joy0right then p1_xspeed = PLAYER_SPEED
   if joy0left then p1_xspeed = 0 - PLAYER_SPEED

   ; Position missile at sprite edge and test
   if p1_xspeed then temp1 = missile0x : temp2 = missile0y
   if p1_xspeed then missile0y = player0y - 6  ; Center Y of sprite
   if p1_xspeed = 1 then missile0x = player0x + 8  ; Right edge
   if p1_xspeed = 255 then missile0x = player0x  ; Left edge
   if p1_xspeed && !collision(missile0, playfield) then player0x = player0x + p1_xspeed
   if p1_xspeed then missile0x = temp1 : missile0y = temp2

   ; Y axis movement: test at sprite's top/bottom edge
   p1_yspeed = 0
   if joy0down then p1_yspeed = PLAYER_SPEED
   if joy0up then p1_yspeed = 0 - PLAYER_SPEED

   ; Position missile at sprite edge and test
   if p1_yspeed then temp1 = missile0x : temp2 = missile0y
   if p1_yspeed then missile0x = player0x + 4  ; Center X of sprite
   if p1_yspeed = 1 then missile0y = player0y  ; Bottom edge
   if p1_yspeed = 255 then missile0y = player0y - 13  ; Top edge
   if p1_yspeed && !collision(missile0, playfield) then player0y = player0y + p1_yspeed
   if p1_yspeed then missile0x = temp1 : missile0y = temp2
...
__Skip_P1_Controls
   ...
   ;***************************************************************
   ;  Handle Player 2 controls - Edge-based collision lookahead
   ;***************************************************************
   ...
   if p2_xspeed then temp3 = missile1x : temp4 = missile1y
   if p2_xspeed then missile1y = player1y - 6  ; Center Y of sprite
   if p2_xspeed = 1 then missile1x = player1x + 8  ; Right edge
   if p2_xspeed = 255 then missile1x = player1x  ; Left edge
   if p2_xspeed && !collision(missile1, playfield) then player1x = player1x + p2_xspeed
   if p2_xspeed then missile1x = temp3 : missile1y = temp4
   ...
   if p2_yspeed then temp3 = missile1x : temp4 = missile1y
   if p2_yspeed then missile1x = player1x + 4  ; Center X of sprite
   if p2_yspeed = 1 then missile1y = player1y  ; Bottom edge
   if p2_yspeed = 255 then missile1y = player1y - 13  ; Top edge
   if p2_yspeed && !collision(missile1, playfield) then player1y = player1y + p2_yspeed
   if p2_yspeed then missile1x = temp3 : missile1y = temp4
...
__Skip_Ball_Move
   ...
   if ball_state = 0 then gosub __Check_Paddle_Collisions bank2
